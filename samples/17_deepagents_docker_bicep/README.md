...
- **Keying and disposal.** The sandbox is keyed by scope, thread and agent identity, acquired on the first operation, here the host's upload, and reused warm after it, and purged at the end by `dispose_scope` — the same call every other sample makes, because LangGraph fires nothing when a thread goes.
