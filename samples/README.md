...
Samples 02, 04, 05 and 07 are not wired into `verify-live.yml`, and only two of them have a reason. Samples 02 and 04 need Windows and a WSL that ships `wslc`, which the Linux runners do not have — that one is structural. Samples 05 and 07 were left out because they needed an endpoint and key CI has no secret for, and since they moved onto the federated Azure credential that objection is gone: 05 needs nothing but a job, and 07 needs a job plus a `docker build` of `images/diagram-sandbox` on the runner. Both are on [#191](https://github.com/sokolaidev/maf-extensions/issues/191). Until then a break in either is invisible until someone runs it by hand, though the docker backend under them is the same one every PR exercises live, `FILES_OUT` path included.
