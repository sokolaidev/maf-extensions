...
| [`08_docker_codeact_files`](08_docker_codeact_files/) | Sample 06 with both of CodeAct's file channels: a `workspace_store` gives `execute_code` a `files` parameter, an `OutputSink` with `CodeactOutputs.DECLARED` gives it an `outputs` one, and the task needs each — no workload code changes, two constructor arguments do | A Docker-compatible engine and an Azure OpenAI deployment (no key — `az login`) |
