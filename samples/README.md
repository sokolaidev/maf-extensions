...
Neither `03_acas_codeact` nor `04_wslc_codeact` is wired into `verify-live.yml` — the workflow covers the Bicep pair only, so a break in either CodeAct sample is invisible until someone runs it by hand.
