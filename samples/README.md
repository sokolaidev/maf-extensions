...
**Six of the eight have a job there, and they pair up**: 01 with 05 on the Bicep workload, 03 with 06 on the CodeAct one, 07 with 08 on reading a file back out. Each pair is one workload against a billable Azure microVM and against a Docker container on the runner, so a red on one side while the other is green names the backend rather than the workload. Only the two ACAS jobs create a billable sandbox; the four docker jobs cost nothing but the model's inference, which no sample avoids — the container is free and the model is reached with a federated credential, so there is no stored secret either.
