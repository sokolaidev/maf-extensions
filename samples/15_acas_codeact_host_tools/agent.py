...
async def _what_one_sandbox_holds(
    ...
    try:
        runs = await sandbox.list_dir(".", working_directory=".")
    ...
    for run in directories:
        try:
            entries = await sandbox.list_dir(f"{run}/host_tools", working_directory=".")
        except FileNotFoundError:
        ...
        files = await sandbox.list_dir(f"{run}/host_tools/{CALLS_DIRECTORY}", working_directory=".")
        left += len(files)
