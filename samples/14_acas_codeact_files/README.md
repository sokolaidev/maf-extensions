...
**The last line of output is the host's, not the model's**, and both of the last two are tagged `[measured]`. Like sample 08, the host explicitly wires `make_file_system_sink(existing="replace")`: a corrected program or a later local execution landing `summary.md` replaces the earlier file. Names not delivered this turn remain in `out/`, including an old summary if no new one lands, so listing the directory cannot prove delivery — the sample prints what the sink actually took *this turn*, as JSON, because a comma is legal in an artifact name. The tag is what stops a model writing that line for it; the reply is filtered before printing, so a line of it starting with the tag comes out quoted ([#314](https://github.com/sokolaidev/maf-extensions/issues/314)).
