...
Successful output includes `[measured] Backend: hyperlight` (or `acas`), the native Hyperlight host (`linux` or `win32`), a CodeAct result with `The workload ran to a definitive result.`, `Result: ok`, and `stdout:` followed by exactly `354224848179261915075`, and a final disposal count. That final count can be zero when per-call cleanup already disposed the sandbox.
