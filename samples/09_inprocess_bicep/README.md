...
A backend with no boundary cannot confine egress, so the one mode it can enforce is `Egress.UNRESTRICTED` — and that is what it declares (`egress_modes = {UNRESTRICTED}`). The Bicep tool is wired to run in that mode (`make_bicep_tools(..., egress=Egress.UNRESTRICTED)`), and because the mode asked and the mode enforced agree, the router serves the pairing. Nothing here wears a `CLOSED` it could not keep: egress is now a mode a workload *runs in*, resolved against what a backend *enforces*, and the router refuses any pairing where the two disagree rather than letting a backend approximate a boundary it does not have ([`egress-resolution.md`](../../docs/sandbox/research/egress-resolution.md)).

Running unconfined is acceptable here for the same reason it always was: the bicep compiler fetches the public module *index* on every `bicep build` and `bicep lint` regardless of the source, so a network is exercised on every run, and the hosts it reaches are Microsoft-operated, public, unauthenticated, with no ARM endpoint and no ambient identity on the host to reach. A no-boundary backend cannot widen what a fixed compiler was already going to touch. That is a property of *this* workload — a fixed compiler, not model-written code — which is why Bicep accepts `UNRESTRICTED` at all and CodeAct does not. The distinction between "said nothing" and "said I confine nothing" that an earlier design carried ([#264](https://github.com/sokolaidev/maf-extensions/issues/264)) is subsumed: a backend's silence is now simply the empty set of enforceable modes.
...
Three diagnostics — the two faults the file carries (`no-unused-params`, `BCP035`) plus the age of the deliberately old API version (`use-recent-api-versions`). The rule ids are opaque tokens the model is instructed to echo verbatim, so their presence is evidence the real compiler ran and its SARIF reached the workload, was parsed, was rendered, and came back through the agent — the whole protocol seam, on the host, with nothing rented. The measured compile count and compiler block prove that the compiler ran. `Disposed N` reports only the final scope purge, which can remove zero sandboxes after per-call disposal.
