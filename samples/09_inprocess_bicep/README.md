...
The day count and the acceptable-version list in `use-recent-api-versions` move with no code change — that one is the drift-prone finding the live check ([`scripts/check_live_sample.py`](../../scripts/check_live_sample.py), shared with samples 01 and 05) deliberately does **not** require. It requires `no-unused-params` and `BCP035` by name, plus an `error` severity (`no-unused-params` is configured to `error` in `bicepconfig.json`), plus the `Disposed` line — the things a healthy run always has whatever the calendar says.
