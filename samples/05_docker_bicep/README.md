...
**Egress is closed, by choice.** This backend has no egress-proxy image configured, so it enforces only `Egress.CLOSED` — a container on `--network none`. The workload is therefore wired to *run* closed (`make_bicep_tools(..., egress=Egress.CLOSED)`): it asks for exactly the mode the backend can enforce, and the router serves it. Had it asked for `ALLOWLIST`, the router would **refuse** the pairing at attach rather than quietly serve it behind a closed boundary — egress is resolved, not degraded ([`egress-resolution.md`](../../docs/sandbox/research/egress-resolution.md)). Running closed, any `br/public:` module reference fails to restore and `bicep_validate` answers with a `MODULE RESTORE FAILED` banner saying type checking of module inputs did not run — loudly wrong beats quietly wrong. `main.bicep` uses no modules, so nothing is restored and this run completes fully offline, which is exactly why this sample validates that file and not one built out of AVM modules.
