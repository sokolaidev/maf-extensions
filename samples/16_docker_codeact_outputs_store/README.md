...
- **`withhold_guest_output=True`** keeps everything the program wrote to `stdout` and `stderr` out of the result. The workspace result includes a completion item, an `ok` or `failed` verdict when an exit status is available, a status report without numeric exit codes or stream sizes, and fixed route guidance. Older published CodeAct releases return the status report and guidance without the separate completion and verdict items. No text the program printed reaches the transcript; file content still can, through the read-back tools below.
