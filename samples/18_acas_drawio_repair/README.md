...
The sample accepts plain-text labels, native geometry and a small allowlist of built-in styles. It checks the authored XML and the converter artifact against the full fixture before writing to storage, including all required components, labels and connections; links, image references, custom fonts, scripts and unknown attributes or styles are refused. This is the sample's fixture policy, not a restriction on the draw.io package's general XML format.
