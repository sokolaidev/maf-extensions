...
- **Lifecycle release is the executor's.** `stop()` and `restart()` condemn the acquired sandbox through the router's unclean path — the contract says `stop` releases resources and `restart` is called when the agent is reset, and a no-op there would leave the container, its filesystem and any running program alive past a `with` block that promised cleanup. A delete that does not land refuses the key rather than leaving the instance reacquirable.
