...
`acquire` is get-or-create, keyed by the caller's scope, thread and agent directory. Reuse between calls depends on the cleanup rung: this sample leaves confinement undeclared, so the framework serialises renders and disposes the sandbox after each call. Docker declares `RECLAIM`, but that alone cannot establish that Graphviz confines its filesystem and process effects to the call's directory.

Each render still needs its own path under `work_dir`: that is where the framework tracks the call's inputs and outputs, and it avoids fixed-name collisions if the workload later establishes confinement and becomes eligible for warm reuse. A real Graphviz filesystem and process probe is still owed before making that claim.

`SandboxToolSession.guest_call_path()` names a directory allocated for this call, under `work_dir`. `sandboxed_tool` performs the selected cleanup when the body returns — after a result, a refusal and an exception alike. Here disposal removes the whole sandbox, including that directory. So:

- **The framework owns serialisation.** The kind needs no lock around renders. Each call has its own guest path, while the host explicitly wires `make_file_system_sink(existing="replace")`, so two renders in one message both land `out/diagram.png` and the second landing replaces the first. A later local execution replaces it too. Replacement is this sample's choice; the sink's default refuses an occupied destination. A stable name is the deliberate half of `DeclaredOutput(name=…)`; the alternative puts the call's id into host storage and into the sentence the model reads.
