...
**And it wires the failure path, which is the half with a decision in it.** `agent.py` builds its router with a `ReclaimConfig`: a `failed_reclaim_policy` and an `on_failure` handler. Both are decisions, and a host leaving them at their defaults has still made them.
