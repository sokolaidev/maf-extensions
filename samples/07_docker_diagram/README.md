...
The factory passes `source_integrity=None`, so the tool declares nothing about its result — a decision, not an omission. `dot`'s failure diagnostic quotes the model's own DOT source back, and which of the reference and the diagnostic comes back is decided by that source — a presence bit — so nothing about the result is first-party; declaring nothing hands it to the framework's input-label join, which finds no labels on `dot: str` and answers untrusted, the same answer both packaged kinds land on.
