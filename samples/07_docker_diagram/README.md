...
On the cleanup-ladder core, this sample leaves confinement undeclared and the framework serialises renders and disposes the sandbox after each call. Warm reuse needs a real Graphviz filesystem and process probe, which is still owed. The published-core behavior described above uses directory reclaim.
