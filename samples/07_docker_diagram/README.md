...
The policy is the one with a blast radius. `DISPOSE` — the default, stated explicitly here rather than inherited — has the framework *try* to end the sandbox when a call's directory could not be removed, so the conversation's next call starts cold; a disposal that does not land is reported as `failed` and leaves the router refusing that key. `KEEP` does not try, and leaves the sandbox warm with the unremovable data still in it. A directory nobody could remove stays readable by every later call in the conversation, because `acquire` is get-or-create, so disposal is the only thing that actually removes it.
...
**`Disposed 0` does not by itself mean the model skipped the tool.** That reading — the T0 behaviour this sample exists to contrast with — holds unless `Reclaim failures this turn` is nonzero, in which case a call the framework could not clean had its sandbox disposed before the purge looked, and the nought at the end is the honest remainder. `ScopeDisposal` is why the count cannot be read alone: it "reads the same whether there was nothing to reclaim or nothing worked".

**A `Not fully disposed:` line is a third thing, and it is weaker than it looks.** It reports that the sweep could not account for everything — not that anything survived. This backend raises `unlisted` whenever its label query fails, which happens whether or not there was a container to find, so the line can appear on a turn that created nothing at all. `check_live_diagram_sample.py` therefore reports it on its own terms, as data that may remain, and lets it excuse nothing about the count: a run that skipped the tool *and* hit a daemon that would not answer is told both things, because both are true.
