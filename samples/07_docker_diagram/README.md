...
**`Disposed 0` does not by itself mean the model skipped the tool.** That reading — the T0 behaviour this sample exists to contrast with — holds unless `Reclaim failures this turn` is nonzero, in which case a call the framework could not clean had its sandbox disposed before the purge looked, and the nought at the end is the honest remainder. `dispose_scope` returns a `ScopePurge`, and its count cannot be read alone: nought reads the same whether there was nothing to take or nothing worked.
