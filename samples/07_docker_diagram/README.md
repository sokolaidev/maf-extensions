...
So a `Not fully disposed:` line leaves the nought **inconclusive** rather than explaining it. The same line is raised by a removal that failed with a sandbox and by a label query that failed without one, and nothing in the output separates them, so `check_live_diagram_sample.py` says the run cannot tell rather than picking one — while still reporting the purge failure in its own right.
