...
**Not the working store, which is narrower than "not a store".** There is a composition this sample declines to draw: a *second* store the model can read and cannot write, with `make_file_store_sink` landing `<call_id>/<name>` into it, so the model reads its own output back with a file-read tool instead of being told by the tool result which names landed. That is the shape [`docs/sandbox/hosts.md`](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/hosts.md) § *A sink the model can read back from* describes, and the reason it is not drawn here is that exposing the second store needs a read-only tool provider of the host's own — a second `FileAccessProvider` contributes a second `file_access_read`, `file_access_ls` and `file_access_grep`, which is a name collision rather than a second store. This sample stays the plain-directory one; the two-store wiring gets a sample of its own.
