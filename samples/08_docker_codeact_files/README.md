...
`make_recording_sink` writes under this directory's `out/` and explicitly chooses `make_file_system_sink(existing="replace")`, so a corrected program or a later local run landing `summary.md` replaces the earlier file. The agent's file store is a separate `InMemoryAgentFileStore`, and **the two are deliberately not the same place**.
...
**Not the working store, which is narrower than "not a store".** There is a composition this sample declines to draw: a *second* store the model can read and cannot write, with `make_file_store_sink` landing `<call_id>/<name>` into it, so the model reads its own output back with a file-read tool instead of being told by the tool result which names landed. That is the shape [`docs/sandbox/hosts.md`](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/hosts.md) § *A sink the model can read back from* describes. Exposing the second store takes a read-only tool provider rather than a second `FileAccessProvider`, which would contribute a second `file_access_read`, `file_access_ls` and `file_access_grep` — a name collision rather than a second store; `sandbox_outputs_read_tools` is that provider. This sample stays the plain-directory one; the two-store wiring gets a sample of its own.
