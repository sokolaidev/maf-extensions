...
The dispatch counterpart is [`samples/15_acas_codeact_host_tools`](../15_acas_codeact_host_tools/) ([#302](https://github.com/sokolaidev/maf-extensions/issues/302)) — a program inside a real microVM calling back out, over a four-stage walk that cannot be reordered. It reports what [#133](https://github.com/sokolaidev/maf-extensions/issues/133) asked for: on the run in its README the dispatched route cost three tool-calling rounds against the direct route's five and wrote none of the data into a call of its own, and paid for it in wall clock — 48s against 15s, on tokens that came out within one per cent of each other.
