...
The dispatch counterpart is [`samples/15_acas_codeact_host_tools`](../15_acas_codeact_host_tools/) ([#302](https://github.com/sokolaidev/maf-extensions/issues/302)) — a program inside a real microVM calling back out, and the round trip measured against the same question answered without one. It reports what [#133](https://github.com/sokolaidev/maf-extensions/issues/133) asked for: on the run in its README the dispatched route cost five times the wall clock and four times the tokens, and was the only one of the two that got the arithmetic right.
