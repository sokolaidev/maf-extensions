...
This is the only sample that reads no configuration — its `verify-live.yml` job is the only one with no `environment:` and no `permissions:` block for the same reason — so the `_scaffold.py` copy every sample carries goes unused here. It stays because [`tests/test_sample_scaffold.py`](../../tests/test_sample_scaffold.py) holds all twelve copies byte-identical, and a sample that dropped it would be the one that drifts.
