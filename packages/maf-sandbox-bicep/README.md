...
The kind declares `confined_to_guest_call_path=True`: it attempts to keep writes inside the call directory and finish its processes with the call. With core 0.38, this metadata is advisory and cleanup defaults to disposal. A host explicitly selecting `Cleanup.RECLAIM` accepts best-effort reuse on a backend declaring `RECLAIM`; cleanup removes the directory, subject to any stricter workload floor. A backend without `RECLAIM` uses reset only where the host permits it and the backend supports it, otherwise disposal. Core 0.37 uses the confinement claim to permit reclamation-based reuse by default. The package's Docker confinement suite measures both rungs against a real engine. Reuse runs the real tool and cleanup through `assert_nothing_left_behind`, checking for changed paths and surviving processes after successful validation, compiler diagnostics, and module restores. The default is measured separately, from the daemon's own container inventory: the sandbox a call ran in is absent afterwards, the next call is served a different instance, and its engine diff against the image is empty on entry — for a clean validation, compiler diagnostics, a module restore, and a cancelled call.
