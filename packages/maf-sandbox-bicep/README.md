...
Build, parameter-file compilation, and lint run with `HOME` set to the call directory, so Bicep's default `.bicep` module cache is removed with that call. A second call restores its modules again. The image's `bicepconfig.json` still supplies the compiler and linter settings through normal source-directory discovery.
