...
Covers only the notice added to `maf_sandbox_docker_sbx/__init__.py` — behavior of the backend
itself is `test_sbx_backend.py`'s job.
"""
...
class TestExperimentalWarningCategory:
    ...
    def test_is_a_user_warning_not_a_future_or_deprecation_warning(self):
        assert issubclass(
            maf_sandbox_docker_sbx.MafSandboxDockerSbxExperimentalWarning, UserWarning
        )
        assert not issubclass(
            maf_sandbox_docker_sbx.MafSandboxDockerSbxExperimentalWarning, DeprecationWarning
        )
        assert not issubclass(
            maf_sandbox_docker_sbx.MafSandboxDockerSbxExperimentalWarning, FutureWarning
        )
...
class TestExperimentalWarningEmission:
    ...
    fresh type object — so a `maf_sandbox_docker_sbx.MafSandboxDockerSbxExperimentalWarning` reference
    ...
    def test_emitted_by_default_on_import(self):
        with pytest.warns(UserWarning, match=r"maf_sandbox_docker_sbx is experimental"):
    ...
    def test_suppressible_via_filterwarnings(self):
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            warnings.filterwarnings(
                "ignore", category=maf_sandbox_docker_sbx.MafSandboxDockerSbxExperimentalWarning
            )
            warnings.warn(
                "maf_sandbox_docker_sbx is experimental and may change or be removed in future "
                "versions without notice.",
                category=maf_sandbox_docker_sbx.MafSandboxDockerSbxExperimentalWarning,
            )
        assert caught == []
...
class TestImportSurvivesDashWError:
    ...
    def test_import_exits_zero_under_dash_w_error(self):
        result = subprocess.run(
            [sys.executable, "-W", "error", "-c", "import maf_sandbox_docker_sbx"],
            capture_output=True,
            text=True,
            timeout=30,
        )
