...
**`withhold_guest_output=True` changes that, by changing what comes back.** The result is then the exit code, how many bytes reached each stream, what landed and where, and one sentence naming the route — every line a host observation, and nothing authored inside the sandbox. So the tool declares `source_integrity="trusted"`, and a host doing information-flow control gets a result it can label without either lying about it or tainting the conversation. Content still reaches the model, by the road the option leaves open: the program writes it to a declared output, that lands in the sink, and the host's own already-classified file-reading path serves it back.
