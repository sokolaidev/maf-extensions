...
Be precise about what that buys. The prose and the shape are this package's, and the artifact names are the model's own — but what fills them is the program's to choose. Whether the program exited cleanly is one bit, and whether it finished in time is another. And **each declared output is one further bit**: the program decides whether to write it, and the result says of every declared name whether it landed, so a call declaring the default eight names carries eight more — which makes `files_out.max_files` the width of the widest channel left. So what stops crossing is guest-authored *text*, not every guest-chosen *bit*. About ten bits a call at the default cap, and a tool-fluent model reads them as its return path rather than stumbling into them ([#859](https://github.com/sokolaidev/maf-extensions/issues/859)). A host that has to close it should not attach this workload at all.
