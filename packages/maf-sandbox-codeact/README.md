...
**Core 0.38 disposes after each call by default.** A host explicitly selecting `SandboxRouter(min_cleanup=Cleanup.RECLAIM, ...)` accepts best-effort reuse with possible residual files and processes, including for CodeAct, when the backend declares `Capability.RECLAIM` and the spec's cleanup floor permits it. Kind confinement metadata is advisory. A host may instead permit `Cleanup.RESET` on a snapshot-capable backend. ACAS declares neither capability and still pays the cold-create cost on each call. Core 0.37 requires the kind's confinement claim for reclamation-based reuse, so it cannot reuse CodeAct that way. See the [cleanup policy](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/tool-call.md#cleanup-as-a-consequence) for the core 0.38 transition in [#1091](https://github.com/sokolaidev/maf-extensions/pull/1091).
