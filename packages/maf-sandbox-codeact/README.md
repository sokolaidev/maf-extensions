...
This package is not affiliated with, endorsed by, or a product of Microsoft — it is a third-party package for [Microsoft Agent Framework](https://aka.ms/AgentFramework).
...
For a runtime with a prepared storage base and no directory-creation support, set `use_call_directory=False` on `CodeactRuntime`. The program receives that base as `guest_call_path` and the bootstrap does not call `makedirs`. Outputs and `outputs.json` stay relative to the base; exclusive admission still applies, and the spec requires at least `Cleanup.RESET`, falling back to disposal when snapshots are unavailable. This mode is part of the execution contract. Hyperlight's output profile uses `guest_work_dir="/output"` and flat filenames with this option.
...
**Core 0.38 disposes after each call by default.** A host explicitly selecting `SandboxRouter(min_cleanup=Cleanup.RECLAIM, ...)` accepts best-effort reuse with possible residual files and processes, including for CodeAct, when the backend declares `Capability.RECLAIM` and the spec's cleanup floor permits it. Kind confinement metadata is advisory. A host may instead permit `Cleanup.RESET` on a snapshot-capable backend. ACAS declares neither capability and still pays the cold-create cost on each call. Core 0.37 requires the kind's confinement claim for reclamation-based reuse, so it cannot reuse CodeAct that way. See the [cleanup policy](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/tool-call.md#cleanup-as-a-consequence) for the core 0.38 transition.
...
**Egress is closed by default, and opening it is the host's decision.** The allowlist has two halves. What the *kind* needs to function is fixed in the package and is empty — nothing in `execute_code` resolves a module or installs anything, so there is nothing for it to need — and it is not configurable, because a deployment able to widen what the kind itself requires could undo the containment. What the *deployment* adds is `make_codeact_tools(egress_allow=…)`, empty by default, for endpoints a published kind cannot know: a package index, an internal artifact store. The spec carries the union, which is what the router matches against the backend and what decides whether this tool is declared as carrying something out. The backend must enforce the exact mode and any requested method scope; a `CLOSED`-only backend refuses a spec that names hosts at attach. The description the model reads names the reachable hosts and methods.

Both factories accept `EgressRule("api.example.com", ("GET",))` beside bare hostnames. Hosts must be schemeless, non-empty and free of whitespace and commas. Write method names uppercase; core 0.39 refuses a lowercase or mixed-case token, because a rule names a verb rather than a spelling. They appear in the tool description. A scoped rule derives `EGRESS_METHODS`, so it is refused at attach unless the backend declares the capability and supports every named token. No shipped backend currently declares it. Method scope narrows the channel and never makes the tool read-only.
...
**The workload claims untrusted in either rendering** — as `source_integrity` on the attached tool where it commits no guidance, and on `maf_sandbox_derived_integrity` where it does, the withholding rendering declaring `source_integrity="trusted"` to the framework so the wrapper owns every item's label. A `"trusted"` claim asks that the result not derive from input the framework has left unestablished, and this one does: what comes back is whatever a model-written `print(...)` chose to emit. Declared rather than omitted, because FIDES reads `source_integrity` as an **override** rather than a floor — it drops the input-label join for it — and that cuts both ways: undeclared, the join answers, and where the arguments carry no labels the host's `default_integrity` does. A host that raised that default would otherwise get a trusted result out of this tool, and nothing here would notice.

**What the untrusted default costs is the model's sight of the result, not the host's sinks.** FIDES hides an untrusted result by default: the item is replaced by a variable reference, and hidden content does not taint the conversation's integrity, so later tools run ungated. Turn hiding off and the result is visible, the conversation goes untrusted, and `PolicyEnforcementFunctionMiddleware` gates every later tool that has not opted in. A host gets one or the other. Three limits on the hiding half: it lapses once anything else has tainted the conversation; it is about integrity only — a hidden item still contributes its confidentiality; and *passing* the reference to another tool is a separate question from the conversation label, gated wherever a host also wires the policy middleware and the destination has opted into neither `allow_untrusted_tools` nor `accepts_untrusted` — and either opt-in exempts a tool from that integrity gate rather than from the middleware, since the confidentiality check still runs. What the gate costs the call is the middleware's configuration: refused under its default, served once a user approves where it asks for approval, served with a warning where it blocks on nothing. The conditions are documented in [`docs/sandbox/information-flow.md`](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/information-flow.md).

**`withhold_guest_output=True` changes what comes back, and not what it claims.** The result is then whether the program exited with status 0, which of the declared outputs landed — by the name the model itself declared — and one sentence naming the route. Neither the exit code nor a stream size is rendered: both are values the program chooses, and neither tells a model anything it can act on without the text. No guest-authored *text* survives into it. What does survive is chosen by the program all the same, so the declaration is the same either way: withholding removes the guest's text from the result, not the guest from the result's derivation. What it is for is the rendering itself — guest-authored text never reaches the transcript, a host's logs, or anything else downstream of the call, and that holds whether or not a host runs information-flow middleware. Content still reaches the model by the road the option leaves open: the program writes it to a declared output, that lands in the sink, and the host's own already-classified file-reading path serves it back.

**Withheld, the result is two items rather than one string, and hiding can reach one without the other.** The call-derived half — the exit line and the landed names, or the exit line alone where a folder replaces them — is stamped `untrusted` beneath the tool's own declaration, its `public` a floor rather than a classification: the framework keeps the stricter of it and the call's own, so it cannot loosen what the call carries; beside it sits the sentence naming the recovery route, which carries that folder, labelled `trusted`, because nothing a call produced reaches it and it is emitted on every return path, refusals included. **Where the conversation is still clean** — hiding is a first-taint protection that stops once anything has tainted the conversation — a FIDES host with hiding on reads the sentence while the rest sits behind a variable reference, which is the point of the split: under one label the sentence went with the call-derived details it was there to explain, and a model got a hidden result and no way to act on it. Where either condition fails both items are simply readable, which is the same outcome one string had. The body leaves the call-derived half unlabelled and core stamps it, because a tool committing guidance must label every item: the host's classification where it set one, `public` as a floor where it did not. The floor invents no host value — the framework keeps the stricter of it and the call's own. A host wiring no information-flow middleware simply reads both items.

The body returns its guidance as unlabelled text; core verifies the commitment and stamps it trusted/public. The withholding rendering is the one that commits guidance, so there core declares `trusted` to the framework and carries this kind's claim on `maf_sandbox_derived_integrity`, stamping every derived item with it. When the host sets the attached tool's `confidentiality` declaration, core also stamps the derived items with that classification and this tool's untrusted integrity. Reading trusted files never promotes them.

On the host-tool-call transport `stderr` is the launcher's field rather than the program's, so its note about the run is surfaced whole under `note:`. Withholding that note would report a run whose output was dropped for its size as one that printed nothing.

Be precise about what that buys. The prose and the shape are this package's, and the artifact names are the model's own — but what fills them is the program's to choose. Whether the program exited cleanly is one bit, and whether it finished in time is another. And **each declared output is one further bit**: the program decides whether to write it, and the result says of every declared name whether it landed, so a call declaring the default eight names carries eight more — which makes `files_out.max_files` the width of the widest channel left. **A producer's note is one more, and it is a size threshold rather than a size.** Every result the host-tool-call transport returns declares `producer_owns_stderr`, and the transport writes a note there for exactly one condition — output that exceeded the cap the host set — so a program picks that bit by choosing how much to print. Its text is a fixed sentence carrying no number; what a *backend* declaring that field puts there is the backend's to bound rather than this kind's, and it is rendered whole because reducing it would report a dropped output as a program that printed nothing. So what stops crossing is guest-authored *text*, not every guest-chosen *bit*. About ten bits a call at the default cap, eleven where a note can appear, and a tool-fluent model reads them as its return path rather than stumbling into them. A host that has to close it should not attach this workload at all.
...
**Calls run one at a time in a sandbox.** The spec asks for exclusive admission, so two `execute_code` calls in one assistant message queue rather than share the sandbox: a model-written program can read whatever a sibling call shared in or wrote, and nothing else keeps it from doing so. A queued call waits `exec_timeout_seconds` plus twice the tool's cleanup bound for each call ahead of it, allowing reclaim or reset followed by disposal, and then answers that the sandbox is busy. Under the default `DISPOSE` cleanup each call pays a disposal, which one call per turn already paid.
