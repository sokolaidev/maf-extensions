...
Be precise about what that buys. The result's *shape* is this package's and its prose is fixed, but three of its values are integers the program chooses — the exit code and the two stream sizes — plus whatever the sink puts in `display`. A program can signal through them: an exit status is 8 bits, and padding its output to a chosen length carries a few more. That is a narrow, per-call channel rather than the open one a rendered `stdout` is, and it is the honest limit of the claim. A host that has to close it should not attach this workload at all.
