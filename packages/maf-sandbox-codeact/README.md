...
**`withhold_guest_output=True` changes that, by changing what comes back.** The result is then the exit code, the size of each stream, what landed and where, and one sentence naming the route. No guest-authored *text* survives into it, so the tool declares `source_integrity="trusted"` and a host doing information-flow control gets a result it can label without either lying about it or tainting the conversation. Content still reaches the model by the road the option leaves open: the program writes it to a declared output, that lands in the sink, and the host's own already-classified file-reading path serves it back.
...
Be precise about what that buys. The prose and the shape are this package's, and the artifact names are the model's own — but three values are the program's to choose: the exit code and the stream sizes. A program can signal through them: an exit status is 8 bits, and padding its output to a chosen length carries a few more. So the claim is that no guest-authored *text* crosses, not that no guest-influenced *bit* does. That is a narrow, per-call channel rather than the open one a rendered `stdout` is, and it is the honest limit. A host that has to close it should not attach this workload at all.
