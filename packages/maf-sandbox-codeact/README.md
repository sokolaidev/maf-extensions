...
**Every call starts from a clean sandbox and gets a directory of its own.** CodeAct explicitly declares `confined_to_guest_call_path=False`: model-written code can write outside that directory and leave processes running, so removing the directory cannot establish a clean sandbox. The router disposes the sandbox after each call unless the backend declares `Capability.SNAPSHOT`, which permits a reset to its clean baseline instead. Files and processes from one call are therefore unavailable to the next, including the next attempt in a fix-round loop.

**Without `SNAPSHOT`, every call pays for a sandbox create.** This includes the cold-create cost on ACAS; a conversation cannot keep a warm CodeAct sandbox between calls. A host cannot lower that cleanup to `RECLAIM`: warm reuse through directory removal requires the kind's confinement claim, and CodeAct makes none. The host's `min_cleanup` may only raise the cleanup floor, for example to require `DISPOSE` even on a snapshot-capable backend.
