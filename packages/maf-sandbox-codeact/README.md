...
**The tool declares no `source_integrity`, in either rendering.** The library's default is `"trusted"`, and what has to hold for it is that the result derives from wholly trusted input. It does not here: what comes back is whatever a model-written `print(...)` chose to emit. Undeclared, MAF's information-flow tracker applies its untrusted default — the fail-safe direction, and the honest one. A declaration would not sit under that default as a floor, either: FIDES reads `source_integrity` as an override and drops the input-label join entirely.
