...
Both factories accept `EgressRule("api.example.com", ("GET",))` beside bare hostnames. Hosts must be schemeless, non-empty and free of whitespace and commas. Method names preserve case and appear in the tool description. A scoped rule derives `EGRESS_METHODS`, so it is refused at attach unless the backend declares the capability and supports every named token. No shipped backend currently declares it. Method scope narrows the channel and never makes the tool read-only.
