...
The inspection contract comes from Microsoft's [WSLC 2.9.3 container implementation](https://github.com/microsoft/WSL/blob/2.9.3/src/windows/wslcsession/WSLCContainer.cpp) and [network implementation](https://github.com/microsoft/WSL/blob/2.9.3/src/windows/wslcsession/WSLCSession.cpp). Container inspection exposes timezone-qualified `Created` and `State.FinishedAt`; it avoids inferring units or inactivity from numeric listing fields. The reaper accepts both 2.9.3 JSON arrays and the JSON-lines listings introduced by newer CLIs, including the [2.9.10 container output](https://github.com/microsoft/WSL/blob/2.9.10/src/windows/wslc/tasks/ContainerTasks.cpp). Offline tests cover these shapes and retention races. Live validation requires Windows with WSLC and `MAF_SANDBOX_WSLC_E2E_IMAGE`; set `MAF_SANDBOX_WSLC_E2E_PROXY_IMAGE` as well for the infrastructure variant, then run `uv run pytest packages/maf-sandbox-wslc/tests/test_wslc_e2e.py -k reap_after_creator_process_exits -q`. These tests create resources in an isolated scope in one process, exit that process abruptly, and sweep in another with a short retention period. They have not yet been run against a real engine for this implementation.
