...
**Windows with WSL 2.9.3 or later.** `wslc` is part of WSL; `wsl --version` reports the version and `wsl --update` moves it forward. There is nothing else to install. The command-line contract this backend depends on — argv passed to `exec` natively, `cp` from a tar on stdin, label filters on `list`, `WSLC_E_*` codes on stderr — was verified against **wslc 2.9.4.0** and again against **2.9.12.0**. Every call spawns `wslc.exe`, so the host's event loop has to be one that can start subprocesses — asyncio's default Proactor loop on Windows does, and a host that installs `WindowsSelectorEventLoopPolicy` has to undo that first, or every acquire fails with a message saying so.
...
**`FILES_IN` has an open check/copy window with root-equivalent placement authority.** On WSLC **2.9.4.0**, a non-root guest that swaps an existing checked parent for a link can redirect `write_file` into a root-owned `0700` directory it cannot write itself. Creating missing children and `prepare_work_dir` (including warm-acquire repair) have the same window. Tar uid/gid stamping changes the resulting ownership, not the authority that places the entries. This is a stated residual, **not a bound or a closure**. Refusal of a link already present during classification does not protect against one planted afterwards. Hosts requiring confinement against concurrent guest mutation must avoid this input plane or select a backend with a supported closure.

The inspected CLI/SDK exposes no constrained upload, guest-authority copy or container freeze. `SIGSTOP` does not freeze all execs, and the remaining guest `test` classifier could not simply run under a future freezer. Cancellation after copy submission can leave partial writes; it is not rollback. The [WSLC measurement record](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/research/wslc-write-window.md) records deterministic existing/missing-parent cases, refusal and cancellation coverage, source versions and mechanism costs.
...
**A listing is read for names and IDs only, in either shape, and nothing else off it.** 2.9.4 emitted a JSON array whose rows carried `Id`, `Name` and a numeric `CreatedAt`/`StateChangedAt`; 2.9.12 emits JSON Lines whose rows carry `ID`, `Names`, a formatted `CreatedAt` string and no `StateChangedAt` at all. Neither version is pinned, so both field spellings are accepted and every remaining field is left to inspection, which has not changed. A payload in neither shape is refused rather than read as a machine with nothing on it — the acquire that would follow creates a container whose name is already taken, and the scope purge that would follow removes nothing and says it succeeded.
...
The same probes were re-run on 2026-09-18 against WSLC 2.9.12.0 and Python 3.13.12, with the whole live suite: 35 passed. The listing timestamps above were not re-measured, because that CLI does not emit them.
