...
def _package_modules():
    """Every module in the installed `maf_sandbox_deepagents`, as `{stem: path}`."""
    import pathlib

    import maf_sandbox_deepagents

    root = pathlib.Path(maf_sandbox_deepagents.__file__).parent  # type: ignore[arg-type]
...
def _declared_import_names():
    """The import names `pyproject.toml` licenses `maf_sandbox_deepagents` to reach for, or `None`.
    ...
    root = pathlib.Path(maf_sandbox_deepagents.__file__).parents[2]  # type: ignore[arg-type]
...
class TestOnlyDeclaredDependencies:
    ...
    def test_every_module_only_imports_what_it_is_declared_to_need(self):
        ...
        if declared is None:
            pytest.skip(
                "pyproject.toml is not next to the installed maf_sandbox_deepagents package — "
                "this check only runs against a source checkout, not an installed-only wheel"
            )

        allowed = set(_sys.stdlib_module_names) | declared | {"maf_sandbox_deepagents"}
        offenders = [
        ...
        assert offenders == [], (
            f"these maf_sandbox_deepagents modules import something outside the standard "
...
class TestNoMafImport:
    """The adapter serves a host that does not run Microsoft Agent Framework at all."""

    def test_the_adapter_does_not_import_agent_framework(self):
        offenders = sorted(
            path.name
        ...
        assert offenders == [], (
            f"these maf_sandbox_deepagents modules import agent_framework: {offenders}. A "
            "Deep Agents host has no framework to import it from."
        )

    def test_the_adapter_does_not_import_a_backend(self):
        offenders = sorted(
            f"{path.name}: {name}"
            for path in _package_modules().values()
            for name in _imported_top_levels(path)
            if name.startswith("maf_sandbox_") and name != "maf_sandbox_deepagents"
        )
        assert offenders == [], (
            f"the adapter imports a backend: {offenders}. It speaks to the router, and the "
            "host registers whichever backend it runs."
        )
