...
**Docker Engine 28.0.0 or newer, to serve `Egress.ALLOWLIST`.** An allowlisted sandbox runs on an internal network whose bridge holds no host address, which is the bridge driver's `gateway_mode_ipv4=isolated` and arrived in that engine. An older daemon rejects the option, and the acquire fails saying so rather than serving the workload on an addressed bridge — a bridge address is a route to the host that the allowlist does not cover. `Egress.CLOSED` creates no network at all and runs on any supported engine.
