...
**Docker Engine 28.0.0 or newer, to serve `Egress.ALLOWLIST` with hosts on the list.** Such a sandbox runs on an internal network whose bridge holds no host address, which is the bridge driver's `gateway_mode_ipv4=isolated` and arrived in that engine. An older daemon rejects the option, and the acquire fails saying so rather than serving the workload on an addressed bridge — a bridge address is a route to the host that the allowlist does not cover. The floor is only for that case, because it is only there that a network is built at all: `Egress.CLOSED`, and an `ALLOWLIST` spec whose `egress_allow` is empty, both run on `--network none` and on any supported engine.
