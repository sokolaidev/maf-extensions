...
#: Every tool the registry was carrying when it sealed.  A registry names its own tools, so
#: these are host vocabulary rather than a guest's spelling, and cross for the reason
#: `HOST_TOOL_NAME` is already safe to key a metric on.
...
#: Which tool call a record came from, which is the join `KEY` cannot make: at the default
#: `IsolationScope.CONVERSATION` the key is identical for every concurrent call on a thread.
#: Distinct from `CALL_ID` above, which is the key's own and exists only at
#: `IsolationScope.CALL` — the two coincide there and nowhere else, so one attribute holding
#: either would answer a different question per scope without saying which.  Generated by the
#: agent framework and naming nobody, so it crosses unhashed.
