...
class TestTheAcquireRecordCarriesThePosture:
    ...
    def test_which_tools_were_callable_is_not_derivable_from_the_folds_beside_it(self):
        """Two registries can fold alike and carry different tools, so the names are their own."""
        recorded = build()
...
class TestARecordNamesTheCallItCameFrom:
    """`maf_sandbox.call.id` is the join the key cannot make.

    At the default `IsolationScope.CONVERSATION` the key is identical for every concurrent call
    on one thread, so without this a pipeline groups a conversation and never a call.
    """
    ...
    def test_a_refused_acquire_carries_it_too(self):
        """The refusal is the record an operator goes looking for, so it wants the join most."""
        recorded = build()
    ...
    def test_the_calls_own_record_always_carries_it(self):
        """The one event where it is never absent: this is the record the others join to."""
        recorded = build()
        recorded.observer.tool_call_ended(
            ToolCallEnded(
                tool="execute_code",
    ...
    def test_a_drain_never_carries_it(self):
        """Its window spans whatever calls ran between two removals, so naming one of them
        would file that call's traffic under another's."""
        recorded = build()
        recorded.observer.egress_observed(a_drain())
        assert f"{NAMESPACE}.call.id" not in recorded.attributes()

    def test_what_happened_outside_a_call_records_no_call(self):
        """A framework reclaim, a thread deletion, a direct consumer of the router: an absent
        id is the honest answer rather than an empty string a query would group on."""
        recorded = build()
        recorded.observer.sandbox_acquired(an_acquire(call=None))
        assert f"{NAMESPACE}.call.id" not in recorded.attributes()

    def test_it_is_not_the_keys_own_call_id(self):
        """Two ids that coincide only at `IsolationScope.CALL`. One attribute holding either
        would answer a different question per scope without saying which."""
        recorded = build()
        recorded.observer.sandbox_acquired(an_acquire(call=A_CALL))
        attributes = recorded.attributes()
    ...
    def test_it_crosses_unhashed_while_the_keys_parts_do_not(self):
        """Generated by the framework and naming nobody, so it is not held back as a key is."""
    ...
    def test_two_calls_on_one_conversation_are_told_apart(self):
        """The whole point: one key, two calls, and the records separate."""
        recorded = build()
