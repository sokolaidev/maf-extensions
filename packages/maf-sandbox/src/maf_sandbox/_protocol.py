...
class Sandbox(Protocol):
    ...
    **Confinement is a duty of all five, and it is not a check on the argument string.**  A
    ...
    check is what catches it, so discharge both halves through your method's own bundle —
    :func:`~maf_sandbox.paths.confine_resolve_guest_write_path`,
    :func:`~maf_sandbox.paths.confine_resolve_guest_read_path`,
    :func:`~maf_sandbox.paths.confine_resolve_guest_list_path` or
    :func:`~maf_sandbox.paths.confine_resolve_guest_delete_path` — rather than by assembling the
    pair yourself.  Each carries what its own method owes about the final component and about
    the working directory, and the two refusals a caller must be able to tell apart are defined
    there.  :mod:`maf_sandbox.conformance` is the same duty as probes, for holding a backend
    that writes its own.  **The stat you hand it must not be answered by the guest** wherever your
