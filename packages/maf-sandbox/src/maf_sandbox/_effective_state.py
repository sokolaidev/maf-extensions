...
``call`` is the one identifier here, and it is the same one every event on the observer seam
carries — so a snapshot joins to the acquire, the host-tool calls and the collections of the
call that produced it, and to the folder a ``per_call`` sink landed that call's artifacts in.
It is generated by the framework rather than drawn from anyone's vocabulary and names nobody
on its own, which is what separates it from the two identifiers below.
...
answer nothing they do not already — which is not true of ``call``, and is why that one is here.
"""
...
class EffectiveState:
    ...
    ``call`` is ``None`` only for an acquire made outside a tool call — a router driven
    directly.  Every acquire the middleware collects was made inside one, so a snapshot that
    reaches session state carries it.
    ...
    #: The tool call this sandbox was served to, joining the snapshot to every event that call
    #: emitted.  ``None`` only outside a call.
    call: str | None
