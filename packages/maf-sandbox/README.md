...
**A run that overruns is stopped, not just reported.** The program is started detached, so the bound expires in the supervisor rather than inside an `exec` a backend could tear down along with its container — which used to mean a timed-out program kept running and the only remedy was disposing the whole sandbox, taking every other call in that conversation with it. The launcher now records the program's pid and the supervisor kills it on the way out, over the same `exec` this transport already runs on: no protocol method, no capability beyond the ones the dispatch path requires. When the kill does not land the message says so — *"could not be stopped, so it may still be running"* — and disposal is then what stops it. Children the program spawned are **not** killed: that needs a process group, which the launcher would have to create with `setsid`, and `setsid` forks when its caller is already a group leader — so the pid the launcher reports would be a short-lived intermediary rather than the session it meant. Getting that right needs measuring against each guest, which is [#437](https://github.com/sokolaidev/maf-extensions/issues/437). The guest now needs `sh`, `nohup`, `printf`, `mv`, `mkdir`, and — new in this release — `rm` and `kill`.
...
**A dispatched run's files are deleted now, and a kind has one call to make.** `dispatch_over_exec` removes its own directory on every exit path; the run directory — the model's shared-in files and its artifacts — is `reclaim_run(sandbox, layout)`, which a kind calls in a `finally` after collecting. **A kind that does not call it keeps today's behaviour for that half**, so nothing breaks, but the run directory goes on accumulating one per call. If your kind reads anything out of the transport's directory after `dispatch_over_exec` returns, it will no longer be there; nothing shipped here does.


**Every dispatch now issues one more `exec`, and a timed-out one issues two.** The extra calls are `rm -rf` for the cleanup and `kill -KILL` for the stop. A backend double that counts `exec` calls, or a guest whose commands are allowlisted, will see them — the fakes in this repository's own codeact suite had to be taught to ignore them. A run's wall clock can also exceed its `timeout` by the reclaim's grace on any path, and by a further stop grace when it overran.
...
**A timed-out dispatch now kills the guest program**, where before it left it running. The message is unchanged when the kill lands — silence means stopped — so a host matching on the old text still matches; what is new is the clause that appears when it does not. If your host watched for the old wording, or disposed the sandbox on every `SandboxProgramTimeout` to reclaim the CPU, that disposal is no longer needed in the ordinary case — the message tells you when it still is. The exit marker's meaning is unchanged: the launcher waits on the program, so the code recorded is still the program's own.
