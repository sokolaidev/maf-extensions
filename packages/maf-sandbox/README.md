...
**`dispatch_over_exec` raises `SandboxProgramTimeout` for its own bound.** A `TimeoutError` from it used to mean either the run running out or a backend bounding one of its own calls, and callers could not tell which. The new type — a `TimeoutError` subclass, so existing handlers keep working — is the first; anything else reaching you is the second and says nothing about whether the program is still running. It carries the program's partial output on `output`.
