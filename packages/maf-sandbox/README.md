...
**If you serve `FILES_OUT`, do not assemble the confinement check yourself.** A path whose *parent* is a link satisfies the file name check and still reads outside, and two backends written independently against the prose shipped that same escape ([#142](https://github.com/sokolaidev/maf-extensions/issues/142)). `maf_sandbox.paths` carries one bundle per method — `confine_resolve_guest_write_path`, `confine_resolve_guest_read_path`, `confine_resolve_guest_list_path` and `confine_resolve_guest_delete_path` — each returning the resolved guest path or raising, and each already carrying what its own method owes about the final component and about the working directory itself. All four run the filesystem path check over your own stat — which must be **unconfined** (it covers the work dir's own ancestors), **no-follow** (a stat that resolves a link describes its target and hides the escape), and **not answered by the guest** wherever your engine offers anything else, since a workload asked to describe its own filesystem can answer falsely and that answer is the one the check trusts. A backend with no other mechanism says so in its own README; the repository's [`tests/test_confinement_stat_source.py`](../../tests/test_confinement_stat_source.py) is what holds it to that.
