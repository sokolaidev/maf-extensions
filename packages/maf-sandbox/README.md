...
**Whichever of the four file capabilities you serve, do not assemble the confinement check yourself.** A path whose *parent* is a link satisfies the file name check and still reads outside, and two backends written independently against the prose shipped that same escape ([#142](https://github.com/sokolaidev/maf-extensions/issues/142)). `maf_sandbox.paths` carries one bundle per confinement policy, and the five methods map onto four: `confine_resolve_guest_write_path` for `write_file` (`FILES_IN`), `confine_resolve_guest_read_path` for `stat_file` and `read_file` (`FILES_OUT`), which share a policy because leaving the final component to the caller is what lets a stat describe a link and a read refuse one on kind, `confine_resolve_guest_list_path` for `list_dir` (`FILES_LIST`) and `confine_resolve_guest_delete_path` for `remove` (`FILES_DELETE`). `FILES_IN` is in `DEFAULT_CAPABILITIES`, so this reaches every backend, including one that serves no pull surface at all. Each returns the resolved guest path or raises, and each already carries what its own policy owes about the final component and about the working directory itself. All four run the filesystem path check over your own stat — which must be **unconfined** (it covers the work dir's own ancestors), **no-follow** (a stat that resolves a link describes its target and hides the escape), and **not answered by the guest** wherever your engine offers anything else, since a workload asked to describe its own filesystem can answer falsely and that answer is the one the check trusts. A backend with no other mechanism takes `stat_by_asking_the_guest` or `stat_by_asking_the_guest_as_root` rather than inventing a spelling — one argv, one parser, and the `-L`-first ordering that keeps a link to a directory from answering `-d` — and says so in its own README; the repository's [`tests/test_confinement_stat_source.py`](../../tests/test_confinement_stat_source.py) is what holds it to that, reading a reach for either — by import or through the module — exactly as it reads a hand-rolled `exec`. The pair is named for what it costs and the two halves are named for reach and not for trust: running the probe as root reads paths the guest's own principal cannot and is no more trustworthy, since the binary answering is the image's either way.
