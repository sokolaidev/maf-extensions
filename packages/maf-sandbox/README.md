...
**7. Attached authority.** A workload explicitly requires `ATTACHED_IDENTITY`, bounds sharing and retention, and names its authority destinations and audiences. The router also enforces the host sharing ceiling and matches every exposed channel to the workload rules. Ambient attachment or exceeded sharing, retention or channel bounds raises `SandboxAttachedIdentityNotPermitted` before cold or warm acquisition. See [Attached authority](#attached-authority) for the full contract.
