...
**`guest_run_layout` refuses inputs it accepted in 0.15, and both refusals are new constraints rather than newly-enforced old ones.** A `run_directory` containing `:` is rejected: the shim's directory now travels to the guest through `PYTHONPATH`, which separates on `:` and cannot quote one, so such a path would reach the interpreter as two entries — the second of them relative, resolved against the directory the guest writes into. If your run directories embed a timestamp, `/runs/2026-08-17T10:30:00Z` is the shape that stops working. And a `program` name is refused when its **stem** matches a module the generated shim imports (`json`, `os`, `time`) or one CPython imports at startup (`encodings`, `site`, `sitecustomize`, `usercustomize`) — the program shares a directory with the shim and that directory is on the path from startup, so such a name is imported instead of the module it stands for, or runs before the program does.

**The launcher rewrites the guest's `PYTHONPATH`.** It prepends the shim's directory and keeps inherited entries only when they are absolute. A relative one resolves against the working directory — which the launcher has just changed to the guest's own — so an image that relies on `.` or a relative entry loses it here. Absolute entries are passed through unchanged, including any that contain glob characters.
