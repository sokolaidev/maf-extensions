...
| `capabilities` | `EXEC, FILES_IN, FILES_OUT, FILES_LIST, HOST_TOOLS` — declares only what it implements today, and the last two are a ceiling `acquire` withdraws on an image whose guest is not root |
