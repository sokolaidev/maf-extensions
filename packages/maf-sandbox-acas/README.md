...
**`write_file` runs as the guest wherever the data plane would act above it.** A parent swapped between the check and the write sends the bytes wherever the link points, and what that costs is decided by the principal rather than by the swap. So `acquire` asks once, where a spec requires `FILES_IN`, whether what the plane lands in that sandbox's base is already the guest program's to rewrite and to remove: where it is — the root image, and the common case — the plane keeps the write, and where it is not, and a guest-authority write lands in that base, every write on that sandbox goes through `maf_sandbox.write_file_over_exec` and arrives guest-owned ([#1131](https://github.com/sokolaidev/maf-extensions/issues/1131)). The road is settled once per sandbox and describes which principal this backend writes as, so a call addressing an absolute working directory outside the base takes the same one. It costs three guest commands against the plane's one round trip, and a write that times out or overruns its capture invalidates the sandbox the way any `exec` failure does.
