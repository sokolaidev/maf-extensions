...
> **Renamed.** This package was published as `maf-sandbox-aca` up to `0.2.3`. ACA is Azure Container *Apps*, the broad service, while this backend targets Azure Container Apps **Sandboxes** — so it gained the `s` the rest of the project already used. PyPI names cannot be reused, so this is a new distribution rather than a continuation, and there is no compatibility shim: `maf_sandbox_aca` and the `Aca…` classes do not forward here.
...
This package is not affiliated with, endorsed by, or a product of Microsoft — it is a third-party package for [Microsoft Agent Framework](https://aka.ms/AgentFramework), built on the [Azure Container Apps Sandboxes](https://learn.microsoft.com/azure/container-apps/sandboxes-overview) preview.
...
**What identity is reachable.** ACAS supports managed identity configured on the sandbox group. The host owns the group's identity assignments, permissions and intended sharing; this backend trusts that configuration without ARM assignment reads or a management-read permission requirement. The host-selected control-plane credential remains in the host. Guest identity surfaces depend on deployment. The router does not apply core attached-identity opt-in or bounds to that group configuration, so the host routes workloads to groups with the intended authority. Lifecycle intervals are applied separately by the backend to each new sandbox; [the lifecycle contract](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/backends/acas.md#lifecycle) describes configuration failures and retention limits. `dispose_scope` deletes sandboxes by service-side label, not the shared group principal, and makes no claim to revoke previously issued tokens.
...
| `stat_file` / `read_file` / `list_dir` | native file operations with path checks that are not held through the operation; concurrent swaps can redirect bytes or metadata outside `working_directory`. Reads refuse classified symlinks/directories and over-cap bytes; stat describes a final symlink. See the residual below |
| `isolation` | `microvm` — the router's default floor, so a host that configures nothing already permits this backend |
| `declarations.capabilities` | `EXEC, FILES_IN, FILES_OUT, FILES_LIST, FILES_DELETE, HOST_TOOLS` are a ceiling. `FILES_IN` always uses guest authority and requires guest write permission and transfer utilities. `acquire` withdraws `FILES_OUT` and `HOST_TOOLS` on a failed guest removal probe and withdraws `FILES_DELETE` unless the file plane confirms the guest removed the probe file |
...
**`write_file` always runs as the guest.** Every call uses `maf_sandbox.write_file_over_exec`; no acquire-time probe or guest output can select the host file API. This holds on cold acquisition, first `FILES_IN` use after warm reuse, and alternate absolute working directories. A parent swapped after the path check may redirect placement within the guest's existing reach; guest authority does not make the check and write atomic or confine a root guest.

**Compatibility change:** writes previously served by the host fallback now fail if the guest cannot perform them. A non-root guest cannot write into a service-created root-owned `0755` base. Supply `sh`, `mkdir`, `mv`, `rm`, `base64` and the bounded-exec capture commands, plus an existing guest-writable directory selected by `SandboxSpec.work_dir`. Bake that directory into the image or create it through guest execution under a writable parent; acquisition may create a missing base as root even below `/tmp`. An alternate writable absolute working directory can accept writes even when the storage base is protected. `FILES_IN` remains available, with permission checked by the actual guest write. A completed permission refusal raises `PermissionError`; missing utilities fail the transfer or execution, with no host retry. The [backend guide](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/backends/acas.md#write_file-always-runs-as-the-guest) records the policy and upstream directory-ownership request.

**The road's cost is per chunk.** It runs two control commands plus one for every 48 KiB of content, so a file under that costs three guest commands (0.44 s against the plane's 0.21 s, measured) and one at the declared 32 MiB `files_in` ceiling costs 685, sequentially; `read_timeout_seconds` bounds the whole transfer, so a large file on a slow link is refused where the plane would have taken it. A deadline that expires *between* commands takes the staged sibling back and raises with the sandbox whole, because nothing was running. A deadline reached while the Azure SDK sleeps on an HTTP 429 `Retry-After` also leaves the sandbox whole: the throttled request answered and no retry started. A command that fails *in flight* or receives another retryable status invalidates and disposes it the way any `exec` failure does, and direct cancellation still disposes it.
...
`EGRESS_METHODS` remains unsupported. The live service denies every verb a rule does not name, including custom ones, but redirects, rule precedence, wildcard overlap and the non-TLS path are unmeasured, so enforcement is not established. Both router matching and direct backend acquisition refuse method-scoped rules with `SandboxCapabilityNotSupported`. A GET-only rule also permits request content; it is not a body-free or read-only channel. The [live measurements](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/research/acas.md) record the distinction.

## Native reads retain a confinement residual

`FILES_OUT` and `FILES_LIST` remain declared with native, host-authority operations. The path checks refuse links they observe; they do not hold the checked filesystem state until the operation finishes. The per-method guarantee is:

| Method | What is checked | What a concurrent swap can return |
| --- | --- | --- |
| `read_file` | Ancestors and the final entry; classified links and directories are refused | Outside bytes through a replaced parent or final file, including bytes the guest cannot read |
| `stat_file` | Ancestors; a final symlink is described as itself | Outside metadata through a replaced parent, labelled with the requested path |
| `list_dir` | Ancestors and the listed directory; returned children must have the requested prefix | Outside names and metadata through a replaced ancestor or directory, still bearing that prefix |

A successful removal-compatibility probe is not a read-authority guarantee, and the conservative non-root `FILES_OUT` refusal does not protect direct method calls. A root image removes the privilege difference, but a swap can still cross the caller's working directory. Byte caps, timeouts and disposal do not close that window.

The native route remains available to avoid adding shell or interpreter dependencies to file retrieval and enumeration, or withdrawing those capabilities from existing workloads. A host requiring a held filesystem boundary must choose a backend with that guarantee. Atomic confinement requires service-side support; [the measured contract and alternatives](https://github.com/sokolaidev/maf-extensions/blob/main/docs/sandbox/backends/acas.md#native-reads-retain-a-confinement-residual) record what is retained here. This is a documented residual, not a claim of atomicity.
