...
**Guest authority is observed at acquire.** The file plane writes as root, while `exec` runs as the image's `USER`. The backend plants a probe file in a fresh root-owned directory under `/`, asks the guest to remove it with `rm`, and checks the result through the file plane. A successful exit or stdout claiming root is insufficient: the file must be gone and its directory must remain. Cleanup runs through the file plane even on cancellation. The probe requires a working `rm`; preparation, execution and observation share a 30-second timeout, followed by separately bounded cleanup.

A completed removal failure refuses `FILES_OUT` and `HOST_TOOLS` and warns an `EXEC`-only workload, since a guest unable to write beside uploaded files cannot create declared outputs or host-tool markers ([#722](https://github.com/sokolaidev/maf-extensions/issues/722)). An inconclusive result serves that functional pair, but **`FILES_DELETE` is refused without an observed removal**: the file plane deletes as the host, so guessing wrong could grant more reach than the guest has. Each sandbox keeps its own verdict for warm reuse; transient failures are retried. An image-level hint can refuse before a create, but every new sandbox that needs the probe must establish its own authority. The verdict is not a numeric uid; the service still does not expose ownership for a per-path gate.
