...
**`write_file` runs as the guest wherever a guest write can land.** A parent swapped between the check and the write sends the bytes wherever the link points, and what that costs is decided by the principal rather than by the swap. So `acquire` asks once, where a spec requires `FILES_IN`, whether a guest-authority write lands in that sandbox's base and whether the utilities the road runs are there; where both hold, every write on that sandbox goes through `maf_sandbox.write_file_over_exec` and arrives guest-owned. It deliberately does not ask whether the plane is already within the guest's reach, which would spare a root image the cost: that answer would arrive as the guest's own stdout with no owner reported for anything host-controlled to stand behind it, so a word the guest printed would be what retained host authority. As asked, **no** to either question keeps the data plane — where this backend already was — so no answer the guest can give raises the authority a write runs at. The road is settled once per sandbox and says which principal this backend writes as, so a call addressing an absolute working directory outside the base takes the same one.
...
**Two shapes keep the plane, and the residual with it — know them before choosing this backend for a non-root image.** A base the guest cannot write into, which `/maf-sandbox/work` on such an image is, since the file plane creates it root-owned; and an image missing `mkdir`, `mv` or `base64`. There the bytes still land root-owned at whatever path a swap chose. It is stated rather than refused for one reason: withholding `FILES_IN` would leave this backend no in-door at all on such an image. A per-component gate is not available here on any image: the data plane's stat payload carries no owner to build one from. **A root image does not remove it**, and the two things it might be wanted for come apart here. Running root removes the *privilege increase* — the bytes land where the guest could have put them itself — and leaves the *confinement failure* exactly as it was: the write still goes outside `working_directory`, which is what a host calling `write_file` with a confined path was promised. A deployment that needs the first can run a root image. A deployment that needs the second cannot get it from this backend on any image, and should treat the sandbox as disposable after a run rather than as a boundary that held.
