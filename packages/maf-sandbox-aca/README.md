...
[Azure Container Apps Sandboxes](https://learn.microsoft.com/azure/container-apps/sandboxes-overview) as a sandbox backend for [Microsoft Agent Framework](https://aka.ms/AgentFramework) agents.
...
This package is the backend only, with no sandbox kind of its own. [`sandbox-bicep`](../sandbox-bicep/) is the first kind that runs on it, written against [`sandbox-router`](../sandbox-router/)'s protocol rather than against this backend.
...
```bash
pip install maf-aca-sandboxes        # the backend only
...
It imports nothing from its host application — only `sandbox-router` and `azure-*` — so moving it to its own repository is a file move plus a dependency line. `src/`, `tests/`, `scripts/` and `pyproject.toml` are already the future repo root. `images/bicep-sandbox/` and `infra/bicep-sandbox/` no longer travel with this package's extraction — they belong to `sandbox-bicep`, the kind that uses them.
