...
**So any touch outside the package your title is about inherits your commit's type — not only a drive-by.** Correcting a stale comment in `maf-sandbox-bicep` inside a `feat!:` commit about `maf-sandbox` released bicep 0.5.0, with a changelog announcing a breaking change that package never received. The version that used to recur — **a test added to another package's suite, for a feature that lives in core** — no longer does: a shared conformance suite written that way touches only those packages' `tests/`, which attribute to nobody, so it needs no split. Anything else outside your package still does, and what you do about it turns on whether your change needs it. An **incidental** touch — a stale comment, a typo, a tidy in a package your work does not depend on — comes out of the pull request and goes in one of its own, as `chore:`, which releases nothing.

**What your change genuinely needs in another package stays in the same pull request.** Wiring a dependent to a surface the same pull request adds to core, and moving the bound that surface requires, are changes that dependent really received, so one title covers both and the title check passes with executable changes in each. **Move both ends of that bound, never the floor alone.** Every dependent declares one string — `maf-sandbox>=0.35.0,<0.36` today — so a floor raised to a coming 0.36 without its ceiling is `>=0.36,<0.36`, which admits nothing and stays unsatisfiable after the core publishes. `set_dependents_range.py` moves the pair for exactly this reason; the ceiling you want is that core's own line, `<0.37` for 0.36, which is what `target_ceiling` returns. This used to be split, and the split bought nothing: a stacked pull request that could not go green until the first was merged, a second review of one change, and a window in which `main` carried half of it. **What has to be ordered is the releases, not the pull requests** — a dependent whose floor names a version goes out after that version does, which the publish job's smoke install enforces rather than trusts: it takes the wheel from `dist/` and resolves *that wheel's own requirements* from the real index with no local fallback, so a floor naming a core PyPI does not carry refuses there. Sequencing them is the maintainer's, in [`RELEASING.md`](RELEASING.md) § *Release order*.

**So write a title that is true of every package the pull request touches.** The type reaches all of them, and so does `!`: a `feat!:` marks a dependent breaking when all it did was adapt. That is the price of keeping a breaking core reshape whole — the shape [#876](https://github.com/sokolaidev/maf-extensions/pull/876) landed in — and the correction for it is a note on the already-released changelog section, below, not a split.

**A floor is the one part of that pull request the index has to catch up with, and only at the upload.** The core wheel built from your branch still carries its pre-release version, because release-please bumps versions only in a Release PR, so a dependent declaring `maf-sandbox>=0.16.0` resolves against nothing on the index — however complete the addition to `maf-sandbox` on the same branch is. What a pull request may do is floor on the *prepared* version: `tests.yml` passes `--overrides "$CORE_OVERRIDE"` to its clean-environment install and `--local-core "$CORE_WHEEL"` to the published-cores gate, so a dependent floored on the release this branch cuts is tested against the artifact the branch itself builds. The upload is the step that still requires a published core, and `publish-packages.yml` passes no flag — so what waits is that dependent's Release PR, not your pull request. Merging that one before the core is on PyPI fails in the publish job rather than shipping a version nobody can install, and that refusal is re-runnable for the same number: publish the core, re-run the upload, and that version goes out. What cannot be undone is the Release and tag release-please created at the merge — Releases here are immutable, so neither can be removed or repointed — but while the retry succeeds they are untidy rather than lost. Only a number that can never get past the gate needs [`docs/maintainers.md`](docs/maintainers.md#why-a-release-exists-before-its-upload-does)'s recovery: annotate that version's changelog gap, and pick the replacement deliberately. [`RELEASING.md`](RELEASING.md) step 2 carries that refusal and step 4 the floor policy, and [`docs/release-compatibility.md`](docs/release-compatibility.md) the gates that make each pairing honest.

**And a samples floor moves on its own, in a pull request merged once every dependent has published.** The range pull request the release workflow opens deliberately carries no `samples/` hunk — `set_dependents_range.py` switches file sets rather than combining them, and touches `samples/` only under `--samples` — because merged with the packages' half they take the whole suite unsatisfiable whenever the core reached the index first: `scripts/check_samples_against_declared_core.py` stops using its `--local-core` escape the moment something published satisfies the floor, and then resolves every sample with the new core pinned from the index and its dependents taken from there too — where each still caps below that core. Every sample naming a dependent goes unsatisfiable, which blocked all five dependent Release PRs on 0.33.0 — the publishing that would have fixed it — and failed seven live samples on 0.34.0. The release workflow opens the samples pull request automatically alongside the range one; merge the range pull request, let every dependent publish, then merge the automated samples pull request. `tests/test_sample_metadata.py` permits the samples to name the release before the current one, which is what makes the gap legal.
...
Each package is self-contained: its own metadata, `ruff`/`pyright`/`pytest` configuration, `LICENSE` and `CHANGELOG.md`. An unreleased `0.0.0` package may omit `CHANGELOG.md` until release-please creates it in its first Release PR; see [Adding a package](RELEASING.md#adding-a-package). The workspace root does not reach into packages, and an sdist has no root to inherit from.
...
- Pin every GitHub Action by commit SHA with the version in a trailing comment. This repository publishes to PyPI; a moving tag is a supply-chain hole. Dependabot moves the pin and the comment together each month, and only while the comment ends with a version tagged at that commit. A checkout sets `persist-credentials: false` unless its job pushes; `tests/test_workflow_supply_chain.py` holds both rules.
