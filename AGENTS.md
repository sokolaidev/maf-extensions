...
**What your change genuinely needs in another package stays in the same pull request.** Wiring a dependent to a surface the same pull request adds to core, and raising the floor that surface requires, are changes that dependent really received, so one title covers both and the title check passes with executable changes in each. This used to be split, and the split bought nothing: a stacked pull request that could not go green until the first was merged, a second review of one change, and a window in which `main` carried half of it. **What has to be ordered is the releases, not the pull requests** — a dependent whose floor names a version goes out after that version does, which the publish job's smoke install enforces rather than trusts: it takes the wheel from `dist/` and resolves *that wheel's own requirements* from the real index with no local fallback, so a floor naming a core PyPI does not carry refuses there. Sequencing them is the maintainer's, in [`RELEASING.md`](RELEASING.md) § *Release order*.
