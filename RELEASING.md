...
- **Its required check starts itself now.** A bot author trips the approval gate, so the run this pull request creates stops at "Approve and run" and the required check never reports from it. `release-please.yml` therefore dispatches `tests.yml` at each pull request it opens, which is not subject to that gate and lands its checks on the same head SHA. The held run stays visible and does not block anything. If you ever need it by hand: Actions → *Tests* → *Run workflow*, at that branch.
