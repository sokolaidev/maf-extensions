...
- **You still have to clear its held run.** A bot author trips the approval gate, so the run this pull request creates stops at "Approve and run" and the required check never reports from it. `release-please.yml` dispatches `tests.yml` at each pull request it opens, which is not subject to that gate and lands the required contexts on the same head SHA — but **a held run blocks the merge whether or not the checks are green**. Measured on the 0.8.0 cycle: both Release PRs had every required context passing, CodeQL passing, no unresolved threads, and zero required approvals, and both still read `BLOCKED` with their `pull_request` runs at `action_required`. So the dispatch buys you the check results early; it does not buy you the click. **Approve and run** on the held run is the step, and [#196](https://github.com/sokolaidev/maf-extensions/issues/196) is where removing it is being decided.
