...
1. **Widen the ceilings** in a `fix:` pull request, so they admit the version about to be released. The type is load-bearing, not taste: `chore:` and `ci:` release nothing here, so a ceiling widened under one of those gives step 2 no Release PRs to merge and the sequence stops with the window still open. All of them together, too — a ceiling widened one package at a time makes any environment holding two of them unsatisfiable. Until this lands the `maf-sandbox` Release PR cannot even go green — the same run builds every package and installs each into a clean environment against `dist/`, and a ceiling that excludes the `maf-sandbox` wheel sitting there resolves the *older* one off PyPI instead and fails on import.
