...
Each pair shares one assertion script, which is what keeps the two sides comparable. `scripts/check_live_sample.py` (samples 01, 05 and 09) is deliberately loose — it matches rule ids and severities, not whole strings, because the diagnostics carry a day count and an API-version list that climb on their own. It makes one exception to that looseness, and it is the one worth knowing about here: it also asserts the compiler found `bicepconfig.json`, because sample 01 boots a **disk image** and a disk image is a snapshot. Overwriting the registry tag it was imported from leaves the running image untouched, so a config baked at a work-dir root the tool no longer writes to keeps booting, lints against the CLI's built-in defaults and passes every other assertion in the script ([#308](https://github.com/sokolaidev/maf-extensions/issues/308)). If that check is what goes red after a `maf-sandbox-bicep` release, the fix is a re-import under a **new** tag, not a change to the sample — see [`images/bicep-sandbox/README.md`](../images/bicep-sandbox/README.md). `scripts/check_live_codeact_sample.py` (03 and 06) has no such drift to allow for — the task has exactly one right answer — so it matches the literal Fibonacci value. The two artifact samples read the landed file instead of the transcript, because a model handed a sentence by the sink can write that sentence whether or not a file landed: `check_live_codeact_files_sample.py` reads the summary's text, `check_live_diagram_sample.py` reads the PNG's own header. Every one of them is a pure function unit-tested on each PR (`tests/test_check_live_*.py`); only the runs that feed them cost anything.
