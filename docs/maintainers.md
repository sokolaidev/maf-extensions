...
The same exception supplies the Release PR's checks, but for a different reason than the tag's — the `GITHUB_TOKEN` rule is not what stops them. A pull request opened by this token *does* start a `pull_request` run; that run is then held at `action_required` (see below), so the required check is never *reported* and `main` refuses the merge. The workflow dispatches `tests.yml` at the branch, and a dispatched run is not held, so it reports against the same commit. It gets you the report and nothing further: the held run keeps blocking the merge after the required contexts are green, so a human still clears it ([#196](https://github.com/sokolaidev/maf-extensions/issues/196)).

Two roads not taken. **Calling the publish job as a reusable workflow** is closed outright: PyPI forbids it — *"Reusable workflows cannot currently be used as the workflow in a Trusted Publisher"* ([warehouse#11096](https://github.com/pypi/warehouse/issues/11096)) — because the job that mints the token must live in the file registered as the publisher. And **a personal access token or GitHub App**, the usual answer elsewhere, is avoided here for the tag: dispatch already gets a publish running without a credential. For the Release PR it is a different question, and the answer moved. Dispatch was expected to make an App unnecessary there too; it does not, because the dispatched check satisfies the branch rule and the *held run* blocks the merge anyway. So an App via `actions/create-github-app-token` is now the only route that removes the human step — prefer it to a PAT, since it is scoped to this repository, has no expiry to forget, and does not inherit an admin's ruleset bypass. It is a stored credential in a repository that has none, which is why it is a decision ([#196](https://github.com/sokolaidev/maf-extensions/issues/196)) rather than a to-do. It would still not fix the ordering.

## A Release PR's checks wait for approval

A Release PR arrives with `Python (pytest + ruff + pyright)` unreported, and `main` requires it. The run does exist: it is queued as `action_required`, because `github-actions[bot]` trips an approval gate. `release-please.yml` dispatches `tests.yml` at every pull request it opens, and a dispatched run is not held, so the required contexts do report — but **the held run still blocks the merge**. Measured on the 0.8.0 cycle: both Release PRs had every required context green, CodeQL green, no unresolved threads and zero required approvals, and both stayed `BLOCKED`. **Approve and run** on the held run is the step, and it is the only one.
