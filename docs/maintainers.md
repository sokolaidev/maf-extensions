...
5. **An identity with a federated credential.** An app registration (or user-assigned managed identity) whose federated credential has **entity type Environment**, environment name **`live-verify`** — which makes its subject `repo:sokolaidev/maf-extensions:environment:live-verify`, issuer `https://token.actions.githubusercontent.com`, audience `api://AzureADTokenExchange`. The `environment: live-verify` on the workflow's job is what puts that subject in the token, so it must match exactly, the same way the PyPI publisher matches on the environment name. (This is the newer Azure form's *immutable* subject only if the repo has opted in; by default GitHub still emits the name-based subject above, so use the form's *Edit* link on the subject if it prefills the `@id` form.)
6. **RBAC for that identity**, since it authenticates as itself with no key: rights to **create sandboxes in the sandbox group**, a pull role on the registry — **`Container Registry Repository Reader`** on a registry in *RBAC + ABAC* permissions mode, or `AcrPull` on a classic-mode one — and an inference role on the model resource — **`Foundry User`** for a Microsoft Foundry resource, or `Cognitive Services OpenAI User` for a plain Azure OpenAI one. Missing any one fails the sample with an SDK error the tool sanitises; the run log has the detail. `Foundry User` also permits `listKeys`, which is the other reason to set `disableLocalAuth: true` above.
