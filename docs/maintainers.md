...
Docker CodeAct samples 06, 08, 16 and 19 require GET-only access to `pypi.org` and `files.pythonhosted.org` for the PyPI index and package downloads. Their jobs build the packaged iron-proxy before running and pass its tag through `MAF_EGRESS_PROXY_IMAGE`. Published runs build from the installed Docker package; branch runs build from the checkout. The samples require `EGRESS_METHODS` and refuse older backends. Their normal result checks verify computation and file handling; they do not independently measure network enforcement.

`verify-live.yml` installs the *published* wheels into a clean environment and runs twenty-two sample jobs, plus one job that is neither — `acas-e2e` runs `maf-sandbox-acas`'s live suite from the checkout rather than from the index, because it tests a backend against the real service rather than asking whether the published set still works. Four of its shared `FILES_OUT` conformance probes require `Capability.FILES_LIST`, so they skip in the Docker live suite. Hyperlight also declares this capability with output files enabled; its flat listing conformance runs in the required Linux backend tests. A fake answers them offline on every pull request, which is a weaker claim than it sounds — twice already, the package's own idea of the payload has disagreed with the service's. Twelve of them are six pairs — each pair one workload against a real Azure sandbox and against a Docker container on the runner, so a red on one side while the other is green names the backend rather than the workload. [`samples/01_acas_bicep`](../samples/01_acas_bicep/) and [`samples/05_docker_bicep`](../samples/05_docker_bicep/) assert the compiler's diagnostics came back; [`samples/03_acas_codeact`](../samples/03_acas_codeact/) and [`samples/06_docker_codeact`](../samples/06_docker_codeact/) assert the 100th Fibonacci number came back from code actually run in the sandbox — together the happy-path half of the sample coverage; [`samples/08_docker_codeact_files`](../samples/08_docker_codeact_files/) and [`samples/14_acas_codeact_files`](../samples/14_acas_codeact_files/) assert that a declared summary came back *out*, read from disk rather than from stdout; and [`samples/15_acas_codeact_host_tools`](../samples/15_acas_codeact_host_tools/) sends a program inside the sandbox back *out* to call a host tool and gates on which route carried the data — run on either backend via `SAMPLE_BACKEND`, a billable ACAS microVM or a local Docker container — with what the round trip costs measured against the same question answered without one. [`samples/20_terraform_validation`](../samples/20_terraform_validation/) adds two pairs, one for each engine, validating the same random-provider module with Terraform and OpenTofu on Docker and ACAS. [`samples/17_deepagents_docker_bicep`](../samples/17_deepagents_docker_bicep/) pairs the other way: sample 05's workload, image and file under a LangChain Deep Agents agent rather than a MAF one, so a red there while sample 05 is green names the adapter rather than the compiler or the container — and [`samples/19_autogen_docker_codeact`](../samples/19_autogen_docker_codeact/) pairs the same way with sample 06, whose task, image and router run under an AutoGen agent's own code-execution tool. The other eight answer to nothing but themselves: [`samples/07_docker_diagram`](../samples/07_docker_diagram/) lands a rendered PNG through a kind the sample writes, 09 runs the Bicep workload against the CLI on the runner, 10 needs no sandbox and no model at all, 11 and 12 exercise the router's own selection and disposal, 13 repairs Bicep over multiple turns, [18 repairs draw.io XML](../samples/18_acas_drawio_repair/) and verifies stored-file read-back and cleanup with per-call timings, and [`samples/16_docker_codeact_outputs_store`](../samples/16_docker_codeact_outputs_store/) is the only one whose guest output is withheld — which is what makes its check the strongest of the set, since it gates a pair: a read whose result was the bytes the sink landed under that call's folder, and the right answer in the reply. Not the answer alone, since a withheld result still carries a bit the program chooses for whether it exited cleanly. A dispatch takes a **`source`** as well: `published`, the default and what every release verification runs, or `branch`, which injects this checkout's packages into each sample instead of resolving them from the index — the pre-merge question rather than the post-release one, and the only way a sample job exercises a change that is not on PyPI yet. A `branch` run skips the resolved-version assertion, because a package's in-tree version is the one it last released and that assertion would pass while something else was under test. It runs on demand (Actions → *Verify (live)* → *Run workflow*) and once after each real publish of `maf-sandbox`, `maf-sandbox-acas`, `maf-sandbox-bicep`, `maf-sandbox-codeact`, `maf-sandbox-deepagents`, `maf-sandbox-docker`, `maf-sandbox-drawio` or `maf-sandbox-terraform` (dispatched by `publish-packages.yml`, which is why those two files know each other). One release is deliberately left out: a `maf-sandbox` publish where no published dependent admits the candidate after the upload dispatches nothing, because there is nothing for a live run to measure — their own publishes dispatch the check, which is when its answer starts meaning something. A **breaking** `maf-sandbox` release is not that case: the breaking flag says nothing about whether the published dependents still import, so the dispatch is decided by that import check in the publish job, not the changelog heading — and a breaking release whose dependents do still admit and import runs the live check all the more. The verdict is measured after the upload, once the core is fully public, so a dependent that admits during the approval window or the upload itself still dispatches the check rather than a `skip` frozen at build. A break found before the upload refuses it; a break found only after the upload ships with the live check dispatched and the break surfaced, since the upload is immutable — the tradeoff accepted here. The publish run's summary says so when a run is skipped, and again when the train is still draining: a release that publishes while some published dependent has not itself published since that core gets a note saying a red in the live check is more likely the order of the train than the code. Publication time rather than a declared floor, because a floor is raised only when a dependent's code needs the version, so an older minimum can be permanent by design. An annotation rather than a gate, because gating the dispatch on a coarser signal is what was removed earlier and the annotation costs nothing either way. Eight jobs create a **billable sandbox**, a PaaS container session rather than a VM you provision, which is why this workflow never runs on a pull request: seven ACAS sample jobs — 01, 03, 14, 15, 18 and both engines of 20 — and `acas-e2e`. `acas-e2e` creates **four** — one shared by its probes, one the lifecycle test disposes as the thing under test, one the prebuilt-image test boots from the service's catalogue, and one the egress leg acquires under an allowlist, since a denied host needs a sandbox that asked for confinement to be denied against — and sample 15 creates two, giving each route its own so neither can read the other's leftovers on legacy transports — it stays two after the transport gained a cleanup, because the sample runs against whichever core and CodeAct versions are published. Sample 18 creates two to four sandboxes in sequence, one for the rejected XML and one per repair attempt. Each sample 20 job creates one sandbox per validation call. Every count here is **spend, not concurrency**: `acas-e2e` acquires its four over the run and holds at most two at a time — the shared sandbox plus whichever of the other three is alive — because the lifecycle test disposes its own inside the test, the prebuilt-image one hangs off a class-scoped fixture pytest finalises when that class ends, and the egress one is requested by the class after it. `acas-e2e` is still the cheapest of the eight per assertion, since it runs nineteen tests and calls no model. The thirteen docker jobs create nothing billable — twelve of them cost model inference and only 12 costs not even that, since the disposal end of the router's decisions needs no model to demonstrate — and seven build their guest image on the runner from this repository (`images/bicep-sandbox`, `images/diagram-sandbox`, `images/terraform-sandbox`) rather than reading an image reference from configuration; 09 needs only the bicep CLI on the runner, and 10 reaches nothing at run time at all. The Docker backend's live coverage runs in `docker-live.yml` on pushes to `main`, daily at 04:50 UTC, and on manual dispatch. No backend is exercised against a real engine by the pull-request gate; authors can dispatch *Docker (live)* against their branch when pre-merge engine evidence is needed. A failed Docker job, including setup failures and timeouts, triggers a separate reporter that opens a tracking issue or comments on the existing open one with the failed run link. Maintainers investigate that issue and close it after verifying recovery; successful runs do not close it automatically. The ACAS backend's live conformance also runs daily on a schedule, in its own workflow — `conformance-live.yml`, one job that is a copy of `acas-e2e` above, four billable sandboxes acquired over the run, no model — so a drift between that backend and the service is found in days rather than at the next dispatch. One job retries: `samples/13_bicep_fix_loop` runs its two-turn loop a second time when the check exits 3, which means every failure belonged to the model's half — either turn — and every deterministic measurement passed. Every other failure, in that job and in all the others, fails on the first attempt. A retry is annotated on the run and the attempt count is in the job summary, so a red is never a silent second chance. wslc has no scheduled leg at all: it needs Windows with a `wslc` that ships it, which no offered runner is, so its conformance runs wherever a developer has one.
...
**Eight of these are secrets and six are required variables, and the split is load-bearing.** Actions redacts a `secrets` value wherever it reaches a log and never redacts a `vars` one. The runner also echoes each step's resolved `env:` block, and a *job-level* block is echoed on the first step — `actions/checkout` — so no step runs early enough to mask a variable with `::add-mask::`. A variable holding an identifier is therefore published to anyone with a GitHub account the moment the job runs; these logs are readable by any signed-in user, this repository being public. The six required variables name public things: a Microsoft-owned endpoint, a public model name, and four `repository:tag` strings. The eight that name *this* estate are secrets.
...
`scripts/check_live_sample.py` (samples 01, 05 and 09) reads `build(name): N diagnostic(s)` and `[level] rule @ file:line` out of that block. It matches rule ids rather than whole strings, because the diagnostics carry a day count and an API-version list that change over time. It also asserts the compiler found `bicepconfig.json`. The Bicep kind stages its packaged policy before compiling; older clients depend on the image's fallback config. For those clients, correcting a stale disk image requires an import under a new tag — see [`images/bicep-sandbox/README.md`](../images/bicep-sandbox/README.md). `scripts/check_live_codeact_sample.py` (03 and 06) looks for the literal Fibonacci value **inside** the block: the number is a constant any model can recite, so the same digits are worth nothing in a reply and everything in the interpreter's stdout. Each script reads one thing loosely and says so — that the reply *names* what the block reports, which is the claim the tool's answer reached the model rather than only the log.
...
Before any ACAS sample or end-to-end job starts, `verify-live.yml` runs [`scripts/check_acas_live_images.py`](../scripts/check_acas_live_images.py) in a blocking prerequisite job. It checks only the images needed by the selected package's jobs; Docker and in-process jobs remain independent. The daily `conformance-live.yml` runs the same check for all live ACAS images, including the CodeAct imports that its tests do not boot. Both use the Azure CLI session established by OIDC and only list images; the check creates no sandbox, imports nothing, and calls no model.

The checker reads `CODEACT_IMAGE` from each sample under test and the prebuilt default from the end-to-end suite without importing their Python modules. For release tags, the checker and its dependencies come from the default branch while those references still come from the tagged source. Missing imports report the image, its consumers, and an import command; missing prebuilt names report the service catalogue. Multiple snapshots matching one reference also fail, because the backend cannot select one unambiguously. An unset `ACAS_SANDBOX_NONROOT_IMAGE` remains a supported skip, recorded in the job summary; set-and-missing fails. Presence does not establish snapshot freshness, non-root behavior, or whether a sandbox will boot: those remain the live tests' job. Updating a registry tag still requires importing a new snapshot under a new build tag.

### Terraform and OpenTofu images

[Sample 20](../samples/20_terraform_validation/) adds four jobs: Terraform and OpenTofu on Docker and ACAS. Each requires the pinned engine's failed validation of a random-provider module, plus per-call disposal. The two ACAS jobs each create at least one billable sandbox, one per tool call. Their `random` profiles contain provider schemas; the AVM catalog image is not required.

Build and push both Linux amd64 images, then import them into the same live-verify sandbox group. The settings below come from that environment's Azure resources; keep their values out of repository files. Authenticate Docker to the configured registry first with `az acr login --name <registry-name>`.
...
for image in maf-terraform:1.16.2-random maf-opentofu:1.12.6-random; do
  ...
  uv run python packages/maf-sandbox-acas/scripts/import_disk_image.py \
    ...
    --image "$ACAS_SANDBOX_REGISTRY/$image"
done
```
...
Dispatch `verify-live.yml` with `package: maf-sandbox-terraform` and `source: published`, then dispatch with an empty package for all samples. Retain both run URLs when recording measurements in the [kind guide](sandbox/kinds/terraform.md). The publish workflow dispatches the four jobs after a Terraform-kind release.

## Cleanup of the live verification group

The independent [`Cleanup (live)` workflow](../.github/workflows/cleanup-live.yml) runs hourly and selects sandboxes in the configured `live-verify` ACAS group that have been continuously `Stopped` for over 24 hours, using the service's `stateDetails.stoppedAt`. Running sandboxes do not qualify, creation age is irrelevant, and missing stop timestamps are retained and reported. The policy applies even without the suite's labels; the group must be dedicated to workloads that accept that stopped retention period. It reuses the existing Azure OIDC login and sandbox group configuration, creates no billable resource, and makes no model call. Images, snapshots, volumes, and the group remain untouched.

Manual dispatch previews by default; disable `dry_run` to apply. Scheduled runs apply automatically once the workflow is on the default branch. Results appear in the execution log and job summary, and incomplete cleanup fails the job. Monitor failures and missed or disabled schedules; the next successful execution retries surviving resources. [`Operating sandbox cleanup`](sandbox/operations.md) documents the standalone script, the ownership boundary, and the timing limits. This workflow is independent of `verify-live.yml` and `conformance-live.yml`, so an interrupted verification run does not prevent later cleanup.
...
### The declared ranges are behind the index

The question above re-resolves *inside* the declared ranges, so **a ceiling below the next minor puts its answer out of reach**: while every package caps at `<1.20`, nothing a re-resolve can touch goes past 1.19.x and the red that would otherwise announce a minor never fires. That is the question working as specified rather than a defect in it, but it left the one mechanism watching the framework quiet exactly when a ceiling was holding something back, and twice the release was found by somebody running the suite for another reason ([#1315](https://github.com/sokolaidev/maf-extensions/issues/1315)). Dependabot has the same blind spot one layer up: it rewrites pinned versions, and every framework declaration here is a range.

[`check_framework_ceiling.py`](../scripts/check_framework_ceiling.py) asks the other half — whether the index carries a release the declared ranges *exclude*. It reads each ceiling off the `pyproject.toml` manifests and places it against what PyPI publishes, so it reds while the re-resolve beside it stays green. A release is announced only once the simple index **and** that version's own document both carry it: a just-published version reaches one endpoint minutes before the other, and a red nobody can reproduce on a re-run is worse than one arriving a run late. The held release is still named in the summary. Pre-releases and yanked releases are never announced, because a range that did not ask for one resolves to neither.
...
## Updating Bicep diagnostic catalogs

[`bicep-catalog.yml`](../.github/workflows/bicep-catalog.yml) checks the latest stable Azure Bicep release weekly and on manual dispatch. It resolves the release to a commit, reads its source archive, and cross-checks linter implementations against the configuration schema. It proposes changes to the packaged `bicepconfig.json`, `compiler_codes.json` and `catalog_source.json` on one reusable branch. A release with unchanged rules, compiler codes and upstream defaults produces no diff. The workflow runs the catalog and Bicep tool tests before opening or updating its pull request; it does not merge or publish. Repository settings must allow GitHub Actions to create pull requests, and a proposal may require the normal **Approve and run** gate.

The configuration is the kind's linter policy and allowed rule vocabulary. Existing settings are preserved; newly added rules start at upstream defaults, including `off` where that is the default. `catalog_source.json` records the upstream defaults separately so the proposed diff reports changed defaults without silently replacing local policy. Review additions, removals and changed defaults before merging. Compiler codes are explicit identifiers extracted from the diagnostic builder, never a numeric range. Neither a catalog update nor a new rule setting installs a newer compiler in the sandbox image.

Run `uv run python scripts/update_bicep_catalog.py --release v0.47.16 --check` to compare the packaged catalog with a particular release. Omit `--check` to write changes, or omit `--release` to inspect the latest stable release. Extraction fails if the upstream schema and rule implementations disagree; a failing workflow needs an extractor update before its catalog can be trusted. Dependabot continues to manage supported dependency manifests; this workflow handles the source-derived catalogs.
...
6. Register it in `release-please-config.json` — **with its `package-name`**. For an unreleased package at `0.0.0`, leave `CHANGELOG.md` and the first `.release-please-manifest.json` entry to release-please, which creates them in the first Release PR; see [Adding a package](../RELEASING.md#adding-a-package). Existing manifest entries must match their package's declared version. `tests/test_release_config.py` checks configuration coverage, bootstrap eligibility, version agreement and unique tag ownership.
