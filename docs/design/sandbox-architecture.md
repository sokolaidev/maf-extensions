...
Two of the seams this section used to name are closed as of this document's current state: the policy used to be one boolean crossed with one frozenset, unable to express "a micro-VM is enough for dev" or "in-process is fine on my machine" — the ordered isolation ladder and the per-host, per-spec floor above replace it; and there was no capability vocabulary, so kind↔backend fit was implicit in prose — `Capability` and the match in "The three checks, two axes" above replace that too. Held against real workloads, what still binds is: there is no way to get a file *out* of a sandbox, and there is no identity vocabulary for on-behalf-of or platform-attached authority. Each of these is taken up, with this document as the baseline, in [`two-axis-sandbox-policy.md`](two-axis-sandbox-policy.md).
