...
> **Status: AS BUILT** — describes what ships on `main` today: `maf-sandbox` 0.4.0, `maf-sandbox-acas` 0.4.0, `maf-sandbox-bicep` 0.3.1, `maf-sandbox-wslc` 0.3.0. The proposed evolution of this design is [`two-axis-sandbox-policy.md`](two-axis-sandbox-policy.md) (tracking [#85](https://github.com/sokolaidev/maf-extensions/issues/85)); this document is its baseline.
...
| `Isolation` | Six rungs, weakest to strongest: `process` / `runtime` / `container` / `hardened_container` / `microvm` / `vm`, declared by the backend | The order lives in `ISOLATION_RANK`, written down exactly once and pinned by an exhaustiveness test; read by the router's minimum-isolation-floor check; the value lives with the backend because that is who knows the truth about itself; an unrecognised value refuses rather than guesses |
| `Capability` | `EXEC` / `RUN_CODE` / `HOST_TOOLS` / `FILES_IN` / `FILES_OUT` / `NETWORK` / `SNAPSHOT` / `ATTACHED_IDENTITY` — declared by the backend, required by a spec | Silence on either side reads as `DEFAULT_CAPABILITIES = {EXEC, FILES_IN}` — what `Sandbox` already obligates. A functionality claim, not a safety one, unlike `Isolation` and `Egress` |
...
All three answer to different owners, and merging isolation into a capability list would be a design error: how strong the boundary must be *here* is the **host's** policy, read from `min_isolation` — and a spec may raise it, never lower it. What a sandbox may reach, and what it must be able to do, are properties of the **workload**, stated in its spec.
...
Two of the seams this section used to name are closed as of this document's current state: the policy used to be one boolean crossed with one frozenset, unable to express "a micro-VM is enough for dev" or "in-process is fine on my machine" — the ordered isolation ladder and the per-host, per-spec floor above replace it; and there was no capability vocabulary, so kind↔backend fit was implicit in prose — `Capability` and the match in "The three checks, two axes" above replace that too. Held against real workloads, what still binds is: there is no way to get a file *out* of a sandbox, and there is no identity vocabulary for on-behalf-of or platform-attached authority. Each of these is taken up, with this document as the baseline, in [`two-axis-sandbox-policy.md`](two-axis-sandbox-policy.md) — and the first of them, getting a file out, is specified in [`files-out.md`](files-out.md).
