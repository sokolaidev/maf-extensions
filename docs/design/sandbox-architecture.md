...
Held against real workloads, the seams that bind are: the policy is one boolean crossed with one frozenset, so "a micro-VM is enough for dev" and "in-process is fine on my machine" are inexpressible; there is no capability vocabulary, so kind↔backend fit is implicit in prose; there is no way to get a file *out* of a sandbox; sandbox identity does not include the kind, so a second kind on one agent would share a sandbox and union the egress lists ([#84](https://github.com/sokolaidev/maf-extensions/issues/84)); and there is no identity vocabulary for on-behalf-of or platform-attached authority. Each of these is taken up, with this document as the baseline, in [`two-axis-sandbox-policy.md`](two-axis-sandbox-policy.md).
