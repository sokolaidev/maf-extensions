...
> **Status: PROPOSED** — tracking issue [#85](https://github.com/sokolaidev/maf-extensions/issues/85). Prerequisite defect, filed separately: [#84](https://github.com/sokolaidev/maf-extensions/issues/84) (sandbox identity must incorporate the kind) — landed, alongside Rollout items 1 (the isolation ladder, the floor, and `deployed`'s removal), 2 (capabilities and matching), 4 (`FILES_OUT` with caps, specified separately in [`files-out.md`](files-out.md) and shipped from there) and 5's safety-contract half and its transport ([#133](https://github.com/sokolaidev/maf-extensions/issues/133) parts A and B — see the status note on the `HOST_TOOLS` section below); the rest of this document is still proposed, and its code blocks outside those items stay design sketches. The baseline this evolves — the architecture as it ships today — is [`sandbox-architecture.md`](sandbox-architecture.md).
