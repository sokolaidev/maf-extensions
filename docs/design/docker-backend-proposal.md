...
**Files out is stat, then the same tar streamed the other way.** The pull surface takes `working_directory` as a parameter exactly as `exec` does — no sandbox object knows the spec's `work_dir`, so the path resolves against the argument and a resolution outside it is refused before any subprocess is built. The contract `files-out.md` sets is **stat, refuse if over cap or of unknown size, then read**, and Docker serves it natively: the spec names `HEAD /containers/{id}/archive`, whose `X-Docker-Container-Path-Stat` carries name, size, Go-mode and `linkTarget` — and a CLI-driven backend has the same daemon-provided answer one layer down, because the **first 512-byte tar header** of `docker cp <name>:<path> -` carries the size, the entry type flag and the link target before any content byte. So `stat_file` reads one header block and reaps the copy, `read_file` stats and then streams the rest of the same tar, and whether that header block remains the mechanism or the `HEAD` endpoint ever earns a minimal API client is the implementing PR's call — the header keeps the driver at zero dependencies, while the endpoint would reintroduce exactly the socket and context handling the driver choice exists to avoid. Enforcement follows: an over-cap or unknown-size output is refused before its content moves, and because the read is a stream this backend also takes the second line the spec encourages, counting tar bytes and killing the `cp` subprocess past the cap — either way a breach fails the whole collection with no partial delivery. Only **regular files** are read, refused twice independently: at stat on the entry kind, and again on the tar entry's type bit, since `docker cp` without `-L` tars a symlink as a link *entry* rather than the target's bytes. And paths follow the spec's one grammar — POSIX-shaped literals, refused on backslashes or traversal — with an absent declared output mapped onto the spec's error taxonomy (absence for `required=False`, a named error otherwise), the exact exception types being the protocol PR's to settle.
...
6. A `_smoke_maf_sandbox_docker()` entry in `scripts/smoke_install.py` that constructs the backend without a client present, asserts `Isolation.CONTAINER`, asserts `FILES_OUT` in the declared capabilities, asserts both egress modes, and — if the proxy hoist below does not happen — asserts the proxy build context shipped in the wheel.
