...
**Files out is the same tar, streamed the other way.** `read_file(path)` becomes `docker cp <name>:<work_dir>/<path> -` — the engine tars the declared file to stdout, which works against an image with no shell at all, the exact property that made this backend the stated reason `files-out.md` split `FILES_LIST` off. Three of that spec's rules bind the implementation directly. Caps are **stream-counted and fail-closed**: there is no stat to pre-check with, so the backend counts tar bytes as they arrive and kills the `cp` subprocess past the spec's cap, failing the whole collection with no partial delivery. Only **regular files** are read, and the refusal is structural: `docker cp` without `-L` tars a symlink as a link *entry* rather than following it, so the backend refuses on the tar type bit, and a link is refused whether or not its target would have resolved inside `work_dir`. And paths follow the spec's one grammar — POSIX-shaped literals relative to `work_dir`, refused on backslashes or traversal before any subprocess is built — with an absent declared output mapped onto the spec's error taxonomy (absence for `required=False`, a named error otherwise), the exact exception types being the protocol PR's to settle.
...
- That a declared output written by the workload comes back byte-identical through the tar stream, that a cap breach aborts mid-transfer with no partial delivery, and that a symlinked output is refused on the tar type bit — the write → exec → read-back round trip and both negative controls.
