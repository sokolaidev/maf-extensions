...
The second refusal has a legitimate case behind it, and `--unreleased-core` is how a pull request says so. A change that adds something to the core **and uses it from a dependent** has to raise that dependent's floor to a release that does not exist yet, so nothing published is admitted — until the core uploads, minutes later, from the same merge. Refusing that would split every such change across two releases. With the flag, which only `tests.yml` passes, the gate reads the branch as well as the index: given the pull request title and its changed paths — the two inputs `check_release_order.py` already predicts a release from — a range that admits nothing published is accepted when the release *this branch would cut* falls inside it. The change must touch the core, the title must name a type that releases, and the predicted version must be inside both ends; any of those missing and the refusal stands. `publish-packages.yml` passes no flag, so before an upload the strict reading is the only one.
