...
The second is right before an upload and wrong on a pull request, where a change that uses a new core symbol from a dependent must floor it on a release that does not exist yet. `--unreleased-core`, which only `tests.yml` passes, lifts that one refusal when the core the range waits for is one the branch already carries — release-please merges the bump before the tag publishes — or one this pull request would cut, predicted from its title and changed paths as `check_release_order.py` predicts. `publish-packages.yml` passes no flag.
