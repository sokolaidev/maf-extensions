...
**Four of the backend's declarations — `capabilities`, `limits`, `egress_modes`, `os_families` — are read off it with `getattr`**, so a backend written before each existed keeps loading, and the router picks what its silence means: charitably for `capabilities` (the default set), conservatively for `limits` (the default ceilings), and as the absence of any claim for the other two — which refuses *every* ask on `egress_modes`, where an empty enforceable set enforces nothing, and only an asking spec on `os_families`, where an empty set is a backend with no guest shape to name. Only `isolation` is a Protocol member read directly, because a backend with no rung at all cannot be placed against a floor. This document used to say three was where the pattern stopped and a fourth was the signal to collapse them into one optional declarations object. The fourth arrived with the guest-shape axis, so the signal has fired and the collapse is owed. It is deliberately not part of the change that tripped the count: the collapse rewrites every backend's declaration surface at once, where the axis that tripped it is additive by construction and breaks nothing.
...
| The four `getattr`-read backend declarations collapse into one optional declarations object | open — the count this document named as the trigger has been reached, and the collapse is owed. This page owns the decision; [`capabilities.md`](capabilities.md) and [`backends/README.md`](backends/README.md) name the count and point here | [#591](https://github.com/sokolaidev/maf-extensions/issues/591) (open) |
