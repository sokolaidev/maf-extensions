...
> The router's policy surface — the isolation ladder, the micro-VM standard, the floor, and the five checks every sandbox request passes. Source of record: [`research/two-axis-sandbox-policy.md`](research/two-axis-sandbox-policy.md).
...
**They were four for most of this package's life, and the fifth is an addition rather than a split.** The guest-shape match arrived with `OsFamily`, and it sits beside the capability match rather than among the safety refusals because it asks the same kind of question — can this backend serve this workload at all — about a different property.
...
**Four of the backend's declarations — `capabilities`, `limits`, `egress_modes`, `os_families` — arrive as one `BackendDeclarations`, read off the backend with a single `getattr`**, so a backend written before the object existed keeps loading and is read as `DEFAULT_BACKEND_DECLARATIONS`. The four silence rules did not merge with the reads: each is a **field default**, and they still differ — charitably for `capabilities` (the default set), conservatively for `limits` (the default ceilings), and as the absence of any claim for the other two, which refuses *every* ask on `egress_modes`, where an empty enforceable set enforces nothing, and only an asking spec on `os_families`, where an empty set is a backend with no guest shape to name. An unstated field therefore means exactly what an absent attribute used to. Only `isolation` is a Protocol member read directly, because a backend with no rung at all cannot be placed against a floor, and stating it twice would give a reader two places to look.

The object replaced four separate `getattr` reads, which this document named as the pattern to stop at — three was where it stopped, and `os_families` was the fourth that fired the signal. **A backend still carrying any of the four attributes is refused when the router resolves it**, with the field named. Nothing else can catch that migration: none of the four was ever a Protocol member, so `isinstance(backend, SandboxBackend)` holds either way and no type checker sees a backend half-moved — while an unnoticed `egress_modes` would be read as silence, which enforces nothing and refuses every workload.

**All five answer to different owners**, and that is why they are five checks rather than one list. How strong the boundary must be *here* is the **host's** policy, read from `min_isolation`, and a spec may raise it and never lower it. What a sandbox may reach, what shape of guest it needs, and what it must be able to do are properties of the **workload**, stated in its spec. Who registers a callable host function, and whose authority it carries, is the host's again — see [`hosts.md`](hosts.md).
