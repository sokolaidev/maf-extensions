...
| A mandatory `Sandbox.reclaim`; core dispatches the removal rather than spelling it | shipped — all four in-tree backends implement the member; wslc refuses it and withholds `RECLAIM`, so its calls resolve to disposal | [#477](https://github.com/sokolaidev/maf-extensions/issues/477) |
