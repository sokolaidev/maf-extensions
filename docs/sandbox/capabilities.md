...
**Backend maxima follow the safety-claim silence rule**, not the capability one: an undeclared `limits` resolves to `DEFAULT_SANDBOX_LIMITS` and a bigger ask is refused with `SandboxTransferLimitsNotPermitted`. That makes `limits` one of **four** optional backend declarations read with `getattr`, beside `capabilities`, `egress_modes` and `os_families` — the count named as the signal to collapse them into one optional declarations object, which [`policy-isolation.md`](policy-isolation.md) owns and `os_families` tripped.
...
| What `reclaim` promises about a link at the directory it is handed | open — the licensing rule is that the caller created it, and a guest can replace it with a link before cleanup runs; every backend unlinks today — docker and wslc via `rm -rf`, acas via the data plane's `delete_file` (which unlinks rather than follows, [#708](https://github.com/sokolaidev/maf-extensions/pull/708)) | [#584](https://github.com/sokolaidev/maf-extensions/issues/584) (open) |
...
| A guest-OS axis, declared and matched | shipped — `OsFamily`, `os_families`, `requires_os_family`, `SandboxOsFamilyNotSupported`, checked at attach and again in `acquire`; released in 0.20.0. No shipped backend declares a family, and what a guest has *installed* is still declared by nobody | [#111](https://github.com/sokolaidev/maf-extensions/issues/111) (closed) by [#532](https://github.com/sokolaidev/maf-extensions/pull/532) (merged); release [#542](https://github.com/sokolaidev/maf-extensions/pull/542) (merged); the remainder is [`guest-platform-and-commands.md`](guest-platform-and-commands.md)'s table |
