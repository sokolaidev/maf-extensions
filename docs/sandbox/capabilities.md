...
**Backend maxima follow the safety-claim silence rule**, not the capability one: an undeclared `limits` resolves to `DEFAULT_SANDBOX_LIMITS` and a bigger ask is refused with `SandboxTransferLimitsNotPermitted`. That makes `limits` one of **four** optional backend declarations read with `getattr`, beside `capabilities`, `egress_modes` and `os_families`. Four is the count named as the signal to collapse all of them into one optional declarations object, and `os_families` is what tripped it. The collapse is owed and is deliberately not done by the change that fired the signal: it rewrites every backend's declaration surface at once, which would make an additive axis breaking.
...
**No shipped backend declares a family**, so the axis refuses nothing today: silence is `frozenset()`, the absence of an answer rather than a default. And the other half of the question is untouched — what is *installed* in a guest is declared by nobody and matched by nothing, and the static ceiling and the acquire-time probe that would answer it are settled and unbuilt in [`guest-platform-and-commands.md`](guest-platform-and-commands.md).
...
|---|---|---|
| Nine-member `Capability`; `DEFAULT_CAPABILITIES = {EXEC, FILES_IN}` | shipped — `NETWORK` removed, the nine that remain and the default unchanged; on `main`, unreleased (0.19.0 predates it and still carries the member) | [#534](https://github.com/sokolaidev/maf-extensions/pull/534) (merged); release [#542](https://github.com/sokolaidev/maf-extensions/pull/542) (open) |
...
| `RUN_CODE` gates a method | shipped — `Sandbox.run_code(code, *, timeout)` and `SandboxQueuedTimeout` on `main`, unreleased (0.19.0 predates them); the three backends' reasoned refusals *are* released, in acas 0.12.0, docker 0.7.0 and wslc 0.10.0, and none declares the capability | [#381](https://github.com/sokolaidev/maf-extensions/issues/381) (closed) by [#532](https://github.com/sokolaidev/maf-extensions/pull/532) (merged); release [#542](https://github.com/sokolaidev/maf-extensions/pull/542) (open); the refusals [#531](https://github.com/sokolaidev/maf-extensions/pull/531) (merged) |
...
| `NETWORK` is removed rather than made matchable; egress is one mode a workload runs in, resolved against the set a backend enforces | shipped — the member is gone from `main` and no spec or backend lost anything, since neither ever used it; unreleased, so the published 0.19.0 enum still has it | [#406](https://github.com/sokolaidev/maf-extensions/issues/406) (closed), umbrella [#265](https://github.com/sokolaidev/maf-extensions/issues/265) (closed) by [#534](https://github.com/sokolaidev/maf-extensions/pull/534) (merged); release [#542](https://github.com/sokolaidev/maf-extensions/pull/542) (open) |
...
| A guest-OS axis, declared and matched | shipped — `OsFamily`, `os_families`, `requires_os_family`, `SandboxOsFamilyNotSupported`, checked at attach and again in `acquire`; on `main`, unreleased (0.19.0 predates it). No shipped backend declares a family, and what a guest has *installed* is still declared by nobody | [#111](https://github.com/sokolaidev/maf-extensions/issues/111) (closed) by [#532](https://github.com/sokolaidev/maf-extensions/pull/532) (merged); release [#542](https://github.com/sokolaidev/maf-extensions/pull/542) (open); the remainder is [`guest-platform-and-commands.md`](guest-platform-and-commands.md)'s table |
| The four `getattr` declarations collapse into one optional declarations object | open — the signal fired when `os_families` made four; not done by the change that fired it, because it rewrites every backend's declaration surface at once | untracked |
