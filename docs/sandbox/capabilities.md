...
| `_resolve` picks a backend per spec (floor ∧ capabilities ∧ egress) | shipped as an **opt-in** — `Selection.PER_SPEC`, registration order as the preference, a route that is a pure function of the spec, and a refusal naming every backend passed over. The default `Selection.FIXED` is unchanged in every respect, so a host that passes nothing sees exactly what it saw. On `main` and unreleased | [#328](https://github.com/sokolaidev/maf-extensions/issues/328) (closed) |
