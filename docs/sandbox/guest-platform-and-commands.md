...
Decision 1 has shipped, and `docker` is the first backend to declare against it. The rows are in the order the work should be done, and the umbrella that carried all of them closed with the first, so each remaining row is pinned on its own.
