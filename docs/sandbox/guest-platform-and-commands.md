...
**The first fork concerns commands used to implement a capability.** ACAS implements `remove` through guest `rm`, so it takes the command branch. Its acquire-time compatibility probe uses the same command. An un-gated member like `reclaim` is narrowed by no probe, because there is no declaration to withdraw.
...
| The static ceiling, then the acquire-time probe | partial — the first acquire-time probe ships, on ACAS, and it narrows on observed removal compatibility: `FILES_OUT` and `HOST_TOOLS` withdrawn on a completed removal failure, `EXEC` warned about, an inconclusive probe served the functional pair ([#722](https://github.com/sokolaidev/maf-extensions/issues/722)) — and `FILES_DELETE` conservatively requires a completed removal. Each removal now executes as the guest; the probe grants no host-plane authority, unlike the earlier gate ([#950](https://github.com/sokolaidev/maf-extensions/issues/950) (closed) by [#962](https://github.com/sokolaidev/maf-extensions/pull/962) (merged)). General command discovery remains unbuilt; ACAS now checks its removal command through file-plane observation. `docker` met the same principal-shaped gap and answered it by removing it rather than by gating: `write_file` stamps the guest's uid and gid on the entries it sends, so the call directory arrives owned by the guest and both capabilities are served on a non-root image ([#680](https://github.com/sokolaidev/maf-extensions/issues/680) (closed) by [#719](https://github.com/sokolaidev/maf-extensions/pull/719) (merged)). A gate there would refuse what works, so its row closed unbuilt. What still earns one is a backend whose file plane **cannot** hand the guest ownership, which is what keeps the ACAS gate right | [#586](https://github.com/sokolaidev/maf-extensions/issues/586) (open); the docker application is [#728](https://github.com/sokolaidev/maf-extensions/issues/728) (closed) unbuilt |
