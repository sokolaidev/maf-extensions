...
| `maf-sandbox-acas`, `_backend.py`, `reclaim` | It is `rm -rf` over `exec` — the SDK's `exec` exposes no user selector, so the `rm` runs as the image's `USER`; the data-plane alternative is tracked in [#707](https://github.com/sokolaidev/maf-extensions/issues/707) |
