...
- Backend declarations are read **synchronously**, with one `getattr` for the whole `BackendDeclarations`, at **two** times — once when the router is constructed, so a declaration this package cannot read fails at startup, and again per spec inside `_refuse_unless_this_backend_can_serve`: `capabilities`, `limits` through `_declared_limits`, `egress_modes` — the set of modes a backend can enforce, not a single declared posture — and `os_families` through `_declared_os_families`. The object must therefore be a plain attribute, or a property over configuration, settled by the time the router asks — never an `async` query.
