...
| `maf-sandbox-wslc` | The write-path filesystem path check is answered by the engine for a directory and a missing path, and by `test` run inside the guest — through core's spelling of it — for which non-directory kind anything else is |
...
| Infrastructure commands raised into protocol methods, `reclaim` first | shipped with documented residuals — `Sandbox.reclaim` ships and core spells `rm -rf` in neither place now. **The write-path filesystem path check is not a command core spells**, and the earlier reading of this row that said so was wrong: it is core's rule in `maf_sandbox.paths` over a stat each backend supplies, and three of the four answer it out of their own engine — a tar header, a data-plane stat, a dict lookup. One backend answers part of it inside the guest, which is the `maf-sandbox-wslc` row above and is a backend's claim rather than core's. The spec base is prepared at acquire through backend file planes. Per-call child creation is still a command, `mkdir -p` in `launcher_script` | [#477](https://github.com/sokolaidev/maf-extensions/issues/477) for the reclaim; the vocabulary, the shared helpers, the contract and the guide have all landed, so the umbrella closes with the guide ([#585](https://github.com/sokolaidev/maf-extensions/issues/585) (closed), its last item [#735](https://github.com/sokolaidev/maf-extensions/issues/735) (closed)); the guest-answered case is [#495](https://github.com/sokolaidev/maf-extensions/issues/495) (closed) by [#1135](https://github.com/sokolaidev/maf-extensions/pull/1135) (merged) — whose third branch, keeping the probe and saying so, is a supported spelling core ships rather than one a backend invents ([#733](https://github.com/sokolaidev/maf-extensions/issues/733) (closed)), and which now asks the guest only what its engine cannot answer, so no reply it gives moves a path. The copy that settles a component now uses private temporary storage removed after the subprocess exits; guest-chosen file sizes still determine transient host-disk use, the documented residual of [#1133](https://github.com/sokolaidev/maf-extensions/issues/1133) (closed) by [#1157](https://github.com/sokolaidev/maf-extensions/pull/1157) (merged); base-directory preparation is shipped for [#466](https://github.com/sokolaidev/maf-extensions/issues/466) (closed) by [#1086](https://github.com/sokolaidev/maf-extensions/pull/1086) (merged); storage ownership and relative addressing are shipped in [#480](https://github.com/sokolaidev/maf-extensions/issues/480) (closed) by [#1090](https://github.com/sokolaidev/maf-extensions/pull/1090) (merged) |
