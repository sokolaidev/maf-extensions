...
**Cost.** The probe runs on cold acquire, and `acquire` is get-or-create with warm reuse, so one batched check amortises across an entire fix-and-retry loop rather than costing a round trip per call. **One exception, and it is the price of failing closed:** a probe that never landed records nothing — deliberately, since a timeout is not a fact about the image — so a sandbox whose probe keeps dropping is re-asked on every warm acquire, at the probe timeout each time. A guest that answers is asked once **per sandbox**, and that is genuinely per sandbox rather than per image — several cold acquires on one unchanged reference each probe the sandbox they booted. A repointed reference adds a second cost on top: a hint left saying root by an older artefact passes every acquire through to a create, and a new sandbox that answers with no uid is refused and discarded with its verdict, so the next acquire pays the same billable create. Deciding which answer is newer needs ordering a probe does not have, which is [#971](https://github.com/sokolaidev/maf-extensions/issues/971). This is the reason an earlier rejection of probing no longer applies: the objection was a per-sandbox round trip, and the lifecycle no longer has that shape. It should still be driven by what the spec asks for, so a spec requiring nothing pays nothing.
