...
| `maf-sandbox-acas`, `_backend.py`, `reclaim` | `delete_file` on the data plane, which acts as the host — `exec` runs as the image's `USER` and the SDK exposes no selector, so `rm -rf` over `exec` leaked on a non-root image ([#707](https://github.com/sokolaidev/maf-extensions/pull/707)) |
