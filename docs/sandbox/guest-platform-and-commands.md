...
| The static ceiling, then the acquire-time probe | implemented — backend-owned command checks narrow requested capabilities at acquire on Docker, WSLC and ACAS; successful checks are cached per physical sandbox. ACAS retains its observed-removal gate and capability-specific inconclusive-result policy. Cleanup capability negotiation remains separate | [#586](https://github.com/sokolaidev/maf-extensions/issues/586) (open) |
