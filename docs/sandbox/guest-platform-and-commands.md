...
**Cost.** The probe runs on cold acquire, and `acquire` is get-or-create with warm reuse, so one batched check amortises across an entire fix-and-retry loop rather than costing a round trip per call. **One exception, and it is the price of failing closed:** a probe that never landed records nothing — deliberately, since a timeout is not a fact about the image — so a sandbox whose probe keeps dropping is re-asked on every warm acquire, at the probe timeout each time. A guest that answers, either with a uid or with something that is not one, is asked once. This is the reason an earlier rejection of probing no longer applies: the objection was a per-sandbox round trip, and the lifecycle no longer has that shape. It should still be driven by what the spec asks for, so a spec requiring nothing pays nothing.
