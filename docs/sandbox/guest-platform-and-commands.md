...
| WSLC | `sh` for `EXEC`; true and false cases of external `/usr/bin/test`, pinned independently of `PATH`, and the image user's `sh` plus `mkdir`, `cat`, `wc`, `mv` and `rm`, for `FILES_IN` — that probe and every write run through the image user's shell. Root `/bin/sh`, `mkdir`, `chown`, `ls` and `pwd`, and a resolved image user, are needed only where a base has to be created, and checked there rather than at acquisition |
