...
| WSLC | `sh` for `EXEC`; true and false cases of external `/usr/bin/test`, pinned independently of `PATH`, and the image user's `mkdir`, `cat`, `wc`, `mv` and `rm`, for `FILES_IN`. Root `/bin/sh`, `mkdir` and `chown`, and a resolved image user, are needed only where a base has to be created, and checked there rather than at acquisition |
