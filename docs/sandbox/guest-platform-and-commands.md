...
| `maf-sandbox-acas`, `_backend.py`, `_AcasSandbox.probe_guest_authority` | The guest has `rm`. The file plane plants a root-owned probe file and verifies its removal; stdout and a successful exit alone cannot enable `FILES_DELETE`. An inconclusive probe serves `FILES_OUT` and `HOST_TOOLS` and refuses `FILES_DELETE` |
...
**The first fork concerns commands used to implement a capability.** ACAS implements `remove` through the data plane's API, so the removal itself needs no guest `rm`. Its acquire-time authority probe does use `rm`, for the separate reason below. Under the tree's command-only wording, only a backend whose declared capabilities are backed by guest commands enters the rest of the tree; an un-gated member like `reclaim` is narrowed by no probe, because there is no declaration to withdraw.

**The fork asks about commands, and a capability can depend on the image without being implemented by a guest command.** ACAS takes the `No` branch above and still probes because its two planes act as two principals: a non-root guest cannot create files inside a root-owned directory the file plane made, which makes `FILES_OUT` and `HOST_TOOLS` unservable there ([#722](https://github.com/sokolaidev/maf-extensions/issues/722)). The probe observes removal authority: the file plane plants a file, the guest attempts its removal, and the file plane verifies the result. It reads no numeric uid. Everything below the third fork still applies: a declared ceiling, a probe at `acquire` when that sandbox has no completed verdict, and a refusal there. **The question the first fork should ask is whether a capability depends on the image at all**, and a guest command is one way for it to.

**A capability can also be unservable for an authority reason even when its mechanism works.** The same authority verdict decides `FILES_DELETE` on ACAS. The data-plane delete works, but can act with more authority than the guest, so it is refused unless the guest's removal passes the observation ([#950](https://github.com/sokolaidev/maf-extensions/issues/950)). One probe supports two refusals with different reasons: a completed removal failure conservatively refuses the functional pair, while any result without an observed removal refuses `FILES_DELETE`. A backend that narrows a ceiling owes the caller the applicable reason.

**The probe's own failure is where the two reasons separate, and a shared answer there is a bug rather than a simplification.** A probe that cannot establish the guest's authority has to decide what an unknown means, and the two rules want opposite answers: the functional one serves, because refusing on an unreadable probe takes a working root image off a deployment and being wrong shows up as a `Permission denied`; the reach one refuses, because an inconclusive probe is the absence of evidence rather than evidence of root and being wrong shows up as nothing at all. **A backend that probes for more than one reason therefore owes a fail-safe direction per reason**, not one for the probe.
...
**Probe by observation, not by announcement.** The backend runs the check and observes the property being tested. For ACAS removal authority, stdout and a successful exit are insufficient: the file plane must confirm that the planted file is gone and its scratch directory remains. An image-provided command cannot establish authority merely by reporting success. A guest that announces a capability it does not have turns a reclaim into a silent no-op, and an image supplied from outside is exactly the one whose self-description should not be trusted.

**Cost.** A completed probe is cached per sandbox, so warm reuse pays no further probe cost. Each cold acquire that needs the probe measures the sandbox it booted, even when another sandbox used the same image reference. ACAS pays file-plane preparation, a guest command, file-plane observation and cleanup for each probe; a spec requiring none of the probed capabilities pays nothing. **Incomplete probes are retried:** a timeout or failed file-plane request records no new verdict, so repeated failures cost another bounded probe on each acquire. A repointed reference can add billable creates too: an earlier positive image hint permits a create, but an inconclusive authority probe refuses a spec requiring `FILES_DELETE` and the newly created sandbox is discarded. The inconclusive result does not demote that earlier image hint, so the next acquire can repeat the create. Ordering those image-level results is the separate limitation tracked by [#971](https://github.com/sokolaidev/maf-extensions/issues/971).
...
| Infrastructure commands raised into protocol methods, `reclaim` first | partial — `Sandbox.reclaim` ships and core spells `rm -rf` in neither place now. **The write-path filesystem path check is not a command core spells**, and the earlier reading of this row that said so was wrong: it is core's rule in `maf_sandbox.paths` over a stat each backend supplies, and three of the four answer it out of their own engine — a tar header, a data-plane stat, a dict lookup. One backend answers it inside the guest, which is the `maf-sandbox-wslc` row above and is a backend's claim rather than core's. The working-directory creation genuinely is still a command, `mkdir -p` in `launcher_script` | [#477](https://github.com/sokolaidev/maf-extensions/issues/477) for the reclaim; the vocabulary, the shared helpers, the contract and the guide have all landed, so the umbrella closes with the guide ([#585](https://github.com/sokolaidev/maf-extensions/issues/585) (closed), its last item [#735](https://github.com/sokolaidev/maf-extensions/issues/735) (closed)); the guest-answered case stays open at [#495](https://github.com/sokolaidev/maf-extensions/issues/495) (open) — whose third branch, keeping the probe and saying so, is now a supported spelling core ships rather than one a backend invents ([#733](https://github.com/sokolaidev/maf-extensions/issues/733) (closed)); the working directory is [#480](https://github.com/sokolaidev/maf-extensions/issues/480) and [#466](https://github.com/sokolaidev/maf-extensions/issues/466) (both open) |
| The static ceiling, then the acquire-time probe | partial — the first acquire-time probe ships, on ACAS, and it narrows on the guest's **authority** rather than on a command: `FILES_OUT` and `HOST_TOOLS` withdrawn on a completed removal failure, `EXEC` warned about, an inconclusive probe served the functional pair ([#722](https://github.com/sokolaidev/maf-extensions/issues/722)) — and `FILES_DELETE` withdrawn beside them on a reach argument rather than a functional one, which is what made the unreadable-probe answer split per reason ([#950](https://github.com/sokolaidev/maf-extensions/issues/950) (closed) by [#962](https://github.com/sokolaidev/maf-extensions/pull/962) (merged)). The command-shaped probe this row was written for, and the ceiling it narrows, are untouched. `docker` met the same principal-shaped gap and answered it by removing it rather than by gating: `write_file` stamps the guest's uid and gid on the entries it sends, so the call directory arrives owned by the guest and both capabilities are served on a non-root image ([#680](https://github.com/sokolaidev/maf-extensions/issues/680) (closed) by [#719](https://github.com/sokolaidev/maf-extensions/pull/719) (merged)). A gate there would refuse what works, so its row closed unbuilt. What still earns one is a backend whose file plane **cannot** hand the guest ownership, which is what keeps the ACAS gate right | [#586](https://github.com/sokolaidev/maf-extensions/issues/586) (open); the docker application is [#728](https://github.com/sokolaidev/maf-extensions/issues/728) (closed) unbuilt |
