...
| `maf-sandbox-acas`, `_backend.py`, `_probe_guest_uid` | The guest answers `id -u`, which the gate on `FILES_OUT`, `HOST_TOOLS` and `FILES_DELETE` reads. An image that cannot answer is served the first two and refused the third — the functional rule fails open, the reach rule fails closed |
