...
The input is an `mxfile` with one to eight uncompressed `diagram` pages, or a bare `mxGraphModel` that the converter wraps in one page. Cell IDs, labels, styles, object metadata, parents and edge endpoint relationships remain native XML. Structural cells `0` and `1` are required on every page. Validation rejects duplicate IDs, missing or cyclic parents, invalid endpoints and invalid supplied geometry in both layout modes. DTDs, entity declarations, compressed pages and unsupported cell/geometry elements are errors. Geometry is validated again after layout and every page must succeed before output is written.
...
Automatic mode supports flat flowcharts and component graphs, with multiple layers, disconnected nodes, cycles, self-loops and parallel edges. Graphviz computes placement and polyline connector points over all page vertices while their layer membership remains unchanged. Only generated identifiers and validated dimensions enter DOT: XML IDs, labels, styles and links are never passed to Graphviz. Missing dimensions default to 160 by 80 units; supplied dimensions remain. Routing attributes and waypoints are replaced, and `childLayout` hints are removed from automatically laid-out pages. Appearance attributes such as colors and arrowheads remain.
...
| --- | --- | --- |
| Model XML produces one editable draw.io artifact through the sandbox output pipeline | implemented; not yet released | untracked |
| Preserve supplied layout by default, automatically lay out missing geometry on each page, and expose a host override | implemented for the flat-graph automatic subset described above | untracked |
