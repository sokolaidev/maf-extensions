...
**A wired registry has no integrity say here.** `HostToolAggregate.result_integrity` is the weakest level over every registered source, and this kind writes no integrity leg for it to weaken — so a registry of trusted sources, a sink-only tool and an unstamped one all read alike. What the aggregate does decide is approval and the outbound cap, both of which the factory still reads off it.
