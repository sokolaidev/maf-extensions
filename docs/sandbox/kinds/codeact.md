...
`make_codeact_tools(withhold_guest_output=True)` keeps the guest's text out and answers with host observations instead: the exit code on every run, the byte count of each stream, whatever `_format_landed` already says about the declared outputs, and one constant sentence naming the route. Every line is something the host saw, so the tool declares `source_integrity="trusted"` — the declaration is the feature, not a side effect of it, because withholding the text and still declaring nothing would leave the framework's untrusted default in place and the conversation tainted exactly as before. Content still reaches the model by the other road: written to a declared output, landed in the sink, and read back through whatever already-classified file-reading path the host has.

Three details are load-bearing.

**The replacement text is not silence.** Measured on the issue: with the streams withheld and nothing said, a failing program's entire result was `exit code: 1`, and the agent burned two further calls debugging blind before recovering on its own. The byte counts and the route sentence are what fill that.

**The model is told in the description**, not left to infer it from a result it cannot use. Told up front, an agent went straight to a declared output and answered in one call, printing nothing at all.

**`CodeactOutputs.DECLARED` is required**, and the other two modes are refused at construction, each for its own reason. `NONE` leaves nowhere for content to go, so no call could return anything usable — the same rule every other impossible pairing in this factory follows. `MANIFEST` is the subtler one: the *program* names its own files in `outputs.json`, and this kind renders those names back into the result, so a guest-chosen string would sit inside a result declaring it holds none. Under `DECLARED` the model names the files, so the names echoed back are its own and the sink's `display` is the host's.

The timeout path needs its own answer, and it is the one place this could have failed open unnoticed. `SandboxProgramTimeout` embeds the program's output *in its message* — not only in `output` — so surfacing the message whole would carry guest text out under a result claiming trusted integrity. Withheld, the sentence is rebuilt from the exception's attributes instead, which is what `signal` is documented for: `"absent"` is its one value asserting a program was never started, so that distinction survives, and every other value is a degree of not knowing and is claimed neither way.
