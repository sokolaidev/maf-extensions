...
| The kind hand-builds its declarations, so a key `sandbox_tool_declarations` learns later skips it | settled — the derivation runs in `sandboxed_tool` and the kind passes a `declarations` mapping for one flow only: a registry that carries something out with no landing artifact to say so, which the derivation cannot see | [#366](https://github.com/sokolaidev/maf-extensions/issues/366) (closed) |
