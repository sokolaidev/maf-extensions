...
`make_codeact_tools(withhold_guest_output=True)` keeps the guest's text out and answers with sizes and the model's own declared names instead: the exit code on every run, the size of each stream, which of the declared outputs landed and which were never written, and one constant sentence naming the route. No guest-authored text survives into the result, so the tool declares `source_integrity="trusted"` — the declaration is the feature, not a side effect of it, because withholding the text and still declaring nothing would leave the framework's untrusted default in place and the conversation tainted exactly as before. Content still reaches the model by the other road: written to a declared output, landed in the sink, and read back through whatever already-classified file-reading path the host has.

**What "trusted" claims, and what it does not.** The prose and the shape are this package's, and the artifact names are the model's own — but what fills them is the program's to choose. An exit status is 8 bits. Each stream's size is a few more, chosen by padding. And **each declared output is one further bit**, because the program decides whether to write it and the result says of every declared name whether it landed — up to `files_out.max_files`, eight by default. So the claim is that no guest-authored *text* crosses, not that no guest-chosen *bit* does. That is a narrow per-call channel against the open one a rendered `stdout` is, and a host that must close it should not attach this workload.
