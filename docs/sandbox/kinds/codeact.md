...
| Result | A [`SandboxResult`](../information-flow.md#the-result-contract): completion, the verdict `ok` or `failed`, the reason a run stopped early, then the program's own text |
