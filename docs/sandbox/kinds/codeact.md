...
**What "trusted" claims, and what it does not.** The result's shape is this package's and its prose is fixed, but three of its values are integers the program chooses — the exit code and the two stream sizes — plus whatever a sink puts in `display`. A program can signal through them: an exit status is 8 bits, and padding output to a chosen length carries a few more. The claim is that no guest-authored *text* crosses, not that no guest-influenced *bit* does. That is a narrow per-call channel against the open one a rendered `stdout` is, and a host that must close it should not attach this workload.
