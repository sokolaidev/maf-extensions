...
**`CodeactOutputs.DECLARED` is required**, and the other two modes are refused at construction, each for its own reason. `NONE` leaves nowhere for content to go, so no call could return anything usable — the same rule every other impossible pairing in this factory follows. `MANIFEST` is the subtler one: the *program* names its own files in `outputs.json`, and this kind renders those names back into the result, so a guest-authored string would sit inside a result rendered to hold none. Under `DECLARED` the model names the files, so the names echoed back are its own.
...
| Withholding guest `stdout`/`stderr` so the result is host-generated, with the mode restricted to `DECLARED` and the timeout message rebuilt from `signal` | shipped | [#769](https://github.com/sokolaidev/maf-extensions/issues/769) (closed) |
| The integrity claim withholding used to carry — `"trusted"` over an exit status, two sizes and a presence bit per output, each chosen by a model-authored program | withdrawn — the rendering shipped as it was and the declaration went, so this kind declares no `source_integrity` in either mode | [#807](https://github.com/sokolaidev/maf-extensions/issues/807) (closed), under [#774](https://github.com/sokolaidev/maf-extensions/issues/774) (open); getting the model its summary back under a label of its own is [#803](https://github.com/sokolaidev/maf-extensions/issues/803) (open) |
...
| The result splits, each item labelled on its own derivation: the standing guidance this package emits on every return path stays visible, while the exit status, the stream sizes and the presence bits are untrusted | not shipped — a kind body returns `str`, which is one item under one label | the mechanism is [#803](https://github.com/sokolaidev/maf-extensions/issues/803) (open), which scopes itself to widening the core return type; **this kind's adoption of it is `untracked`** and wants its own issue once the mechanism lands |
