...
- **Egress stated positively in the spec.** `egress_allow` is an allowlist and everything unlisted is denied, so a kind that forgets the field gets the closed configuration rather than the open one. What the *kind* needs to function is fixed in the package, not left to configuration: a deployment able to widen that could undo the containment the design rests on. See [`../network.md`](../network.md).
...
| Kind | Tool | `requires` | Egress | Package |
|---|---|---|---|---|
| [`bicep`](bicep.md) | `bicep_validate` | `{EXEC, FILES_IN}` — the protocol default, left unaltered | the four hosts an AVM module restore reads, fixed in the spec | [`maf-sandbox-bicep`](../../../packages/maf-sandbox-bicep/README.md) |
| [`codeact`](codeact.md) | `execute_code` | `{EXEC, FILES_IN}`, grown by `FILES_OUT` and `HOST_TOOLS` as the host wires channels | `()` — nothing, unless a deployment names hosts | [`maf-sandbox-codeact`](../../../packages/maf-sandbox-codeact/README.md) |
...
| Fixed templates, listing-pinned paths, sanitized surfaces, positive egress, visible degrades | shipped — the pattern holds in both kinds | — |
...
| A guest-OS axis a kind can declare and a backend match — codeact execs `python3` and nothing states it | open — design settled in [`../guest-platform-and-commands.md`](../guest-platform-and-commands.md) (the interpreter is the `RUN_CODE` road, infrastructure commands become protocol methods, workload commands stay with the kind plus `requires_os_family`); nothing implemented | [#111](https://github.com/sokolaidev/maf-extensions/issues/111) (open) |
| `egress_allow` distinguishes "this kind needs no network" from "nobody asked", so a deployment default has somewhere to live | open | [#403](https://github.com/sokolaidev/maf-extensions/issues/403) (open) |
