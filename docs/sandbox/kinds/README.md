...
| A kind may expose more than one tool name | open — `sandboxed_tool` documents the once-per-tool-and-concatenate pattern, no shipped kind uses it, and the `Tool` column above still assumes one. Siblings sharing a kind share the sandbox, so their specs must agree on image and egress; only the egress half is enforced, and the three backends enforce it three different ways | [`../research/many-tools-one-kind.md`](../research/many-tools-one-kind.md) |
