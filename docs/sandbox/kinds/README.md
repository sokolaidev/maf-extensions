...
| A guest-OS axis a kind can declare and a backend match — codeact execs `python3` and nothing states it | open — deliberately unbuilt while every shipped backend runs a Linux guest, so an axis no backend lacks would refuse nothing | [#111](https://github.com/sokolaidev/maf-extensions/issues/111) (open) |
