...
Obtain the sandbox through `session.acquire` and keep owned files beneath `session.guest_call_path`. Core applies cleanup when the body returns or raises. Do not dispose the sandbox yourself or start work that outlives the call. Leave `confined_to_guest_call_path` at its default `False` until real-backend filesystem and process probes establish the stronger claim needed for warm reuse; writing the input beneath that path alone does not establish it. [Tool-call lifetime](../tool-call.md) owns the full cleanup contract.
