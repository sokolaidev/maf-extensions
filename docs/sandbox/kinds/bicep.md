...
| The tool says `untrusted` outright rather than reaching it through the host's default and the shape of `files: list[str]` | **shipped** — `source_integrity="untrusted"` is passed at the `sandboxed_tool` call. A tier-2 declaration replaces the input-label join and the host's `default_integrity` rather than flooring them, so the answer no longer depends on either. The store channel this row used to defer to is unchanged and still ruled on by [`../information-flow.md`](../information-flow.md), with [`../research/laundering-into-the-file-store.md`](../research/laundering-into-the-file-store.md) as the measured chain | [#840](https://github.com/sokolaidev/maf-extensions/issues/840) (closed) |
