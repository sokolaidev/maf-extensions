...
| An item's label is derived from the integrity of the files it read — `weakest_integrity` folded over the `ListedFile` entries `list_files` answered with | not shipped — this kind reads through `SandboxToolSession.read_file`, so the labels arrive, and it folds none of them. The split it was waiting on is the row below and has shipped, so what remains is the fold | [#881](https://github.com/sokolaidev/maf-extensions/issues/881) (open) |
| The result splits: standing guidance is labelled `trusted` and stays visible, while the diagnostics and their count carry no label of their own and take the call's — untrusted by default, though `default_integrity` is the host's to move | **shipped** — two items on every return path, refusals included, appended at one funnel so no `return` can omit the sentence. `TestTheResultSplits` walks every path and `TestWhatAFidesHostSeesOfASplitResult` drives the real middleware, including the counterfactual: the same declaration over one string hides the sentence with the diagnostics. The sentence is not yet **committed** at attach — `sandboxed_tool(standing_guidance=…)` is core this package's declared floor does not carry, so the every-path rule is held by the funnel rather than by the wrapper | the mechanism is [#803](https://github.com/sokolaidev/maf-extensions/issues/803) (closed) by [#849](https://github.com/sokolaidev/maf-extensions/pull/849) (merged), which scoped itself to the core surface; this kind's adoption is [#852](https://github.com/sokolaidev/maf-extensions/issues/852) (closed) by [#925](https://github.com/sokolaidev/maf-extensions/pull/925) (merged); committing the sentence is [#858](https://github.com/sokolaidev/maf-extensions/issues/858) (open) |
