...
| The result splits, each item labelled on its own derivation: standing guidance stays visible while the diagnostics and their count are untrusted | not shipped here — core takes a list of items from a body now, and this kind still answers with one string | the mechanism is [#803](https://github.com/sokolaidev/maf-extensions/issues/803) (closed) by [#849](https://github.com/sokolaidev/maf-extensions/pull/849) (merged), which scoped itself to the core surface; this kind's adoption is [#852](https://github.com/sokolaidev/maf-extensions/issues/852) (open), which asks first whether a sentence the model can read is worth having when the diagnosis stays hidden |
