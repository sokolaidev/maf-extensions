...
| `declarations` | **empty** — no `source_integrity` and no confidentiality cap. The compiler is first-party and deterministic, but what it is deterministic *about* is a template the model wrote, so the result does not derive from wholly trusted input; the framework's untrusted default is what applies, and it is the honest reading (below). That default is the *host's* `default_integrity` rather than a constant, so the answer is reached rather than declared — saying `untrusted` outright is [#840](https://github.com/sokolaidev/maf-extensions/issues/840). No cap because a host's confidentiality tiers are the host's classification, and declaring one here can activate a policy leg a given host keeps dormant. `TestFidesDeclarations` pins the resulting dict |
