...
| A refusal names the position of a rejected `files` entry rather than quoting a value the framework rewrote | shipped — the framework expands a `[var_id]` reference into `files` before the body runs, so the tool asks it which entries it rewrote (`values_holding_hidden_content`) and renders `files[i]` for those. Where no middleware is reachable a bound on shape stands in, which is weaker: an instruction can be written without spaces | [#810](https://github.com/sokolaidev/maf-extensions/issues/810) (closed) |
