...
| A refusal names the position of a rejected `files` entry rather than quoting a value that does not read like a file name | shipped — the framework expands a `[var_id]` reference into `files` before the body runs, so quoting the argument hands back content it had hidden; an ordinary misspelling still reads back verbatim, and the bound is on shape rather than a closure — an instruction can be written without spaces | [#810](https://github.com/sokolaidev/maf-extensions/issues/810) (closed) |
