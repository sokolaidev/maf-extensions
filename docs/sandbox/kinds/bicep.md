...
The environment argument for `"trusted"` is all true and none of it is the question: the compiler is Microsoft's, the sandbox carries no ambient identity, and nothing is reachable but the four restore hosts. What the compiler is deterministic *about* is the model's own Bicep, and that reaches the rendered result in two places. The **message** is passed through verbatim, and Bicep quotes the source in it — `BCP057` is "The name … does not exist in the current context", where the name is whatever the model wrote. The **file name** is the model's `files` argument, rendered into every location: `[warning] BCP035 @ main.bicep:31: …` names a string the model chose.
