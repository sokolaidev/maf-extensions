...
`reclaim` remains served with `rm -rf` over `exec`, not through the `delete_file` its `remove` goes through. The service's final-component and interior-link behavior is now measured, but moving reclamation to the data plane is tracked separately in [#707](https://github.com/sokolaidev/maf-extensions/issues/707).
...
| ACAS declares `FILES_DELETE` | shipped — live measurement confirmed final-component and interior links are unlinked without following their targets; symlinked parents remain refused because the service resolves them | [#589](https://github.com/sokolaidev/maf-extensions/issues/589) (closed) |
| `reclaim` served over `exec` rather than the data plane's `delete_file` | shipped for now — moving it to the data plane is tracked separately after the service behavior was measured | [#707](https://github.com/sokolaidev/maf-extensions/issues/707) (open), [#477](https://github.com/sokolaidev/maf-extensions/issues/477) |
...
| Which principal a file-plane call acts as | open — unmeasured here. `write_file` reaches the service's data plane with the host's credential while `exec` is the service's own exec API; `reclaim` is `rm -rf` over that `exec`. `remove` is `delete_file` on the data plane rather than a shell, so `reclaim` is the only member with the split — the shape [`docker`](docker.md) had before [#684](https://github.com/sokolaidev/maf-extensions/pull/684) fixed it there. Every image this repository runs is root's, which is what keeps it invisible | [#695](https://github.com/sokolaidev/maf-extensions/issues/695) (open) |
