...
| Shared egress probes — every backend that can enforce `ALLOWLIST` declares it, one has been seen enforcing it | shipped: `run_egress_probes` and `assert_egress_conformance` are in `maf_sandbox.conformance` and all three backend e2e suites call them. ACAS is measured for the first time, so the count of backends a job in this repository sees enforcing goes from one to two — docker on every pull request, ACAS against the real service nightly and after a release — while wslc's leg is written and runs only where a developer has Windows and WSL, as its whole suite always has | [#402](https://github.com/sokolaidev/maf-extensions/issues/402) (open) — the probe [#547](https://github.com/sokolaidev/maf-extensions/pull/547), the e2e wiring [#548](https://github.com/sokolaidev/maf-extensions/pull/548) (open) |
