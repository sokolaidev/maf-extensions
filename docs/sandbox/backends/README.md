...
| `os_families`, matched against a spec's `requires_os_family` | shipped in core, **declared by nobody** — no shipped backend states a guest family, so the axis refuses only a spec that asks | [`../guest-platform-and-commands.md`](../guest-platform-and-commands.md) § Status, first row, which carries the closed issue and the merged PR, and the remaining per-backend rows |
