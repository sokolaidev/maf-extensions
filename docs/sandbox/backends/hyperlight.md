...
The [AKS investigation](../research/hyperlight-aks-integration.md) records the device-plugin/CDI audit and a pinned Python SDK proof. The [approved AKS KVM verification](../research/hyperlight-aks-kvm-verification.md) measures non-root device injection, VM creation, Python execution/reset and one controlled plugin replacement on Azure Linux. AKS Automatic requires an infrastructure-namespace exception for the plugin; application admission stays restricted. These SDK measurements do not establish adapter lifecycle conformance or MSHV support. Container worker containment gates AKS adapter support; replicated deployments additionally require owner routing and fencing.
...
| AKS hosting | KVM device/VM/SDK proof and controlled plugin replacement measured with a temporary infrastructure exception; adapter and broader lifecycle validation remain open | [#1230](https://github.com/sokolaidev/maf-extensions/issues/1230) (open); manifests [#1237](https://github.com/sokolaidev/maf-extensions/issues/1237) (open), container containment [#1238](https://github.com/sokolaidev/maf-extensions/issues/1238) (open), replica ownership [#1239](https://github.com/sokolaidev/maf-extensions/issues/1239) (open) |
