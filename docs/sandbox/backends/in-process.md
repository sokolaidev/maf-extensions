...
The fake implements the whole `Sandbox` protocol, because a member it did not implement would be a member no kind's test suite could exercise: `write_file` ([`testing.py:179`](../../../packages/maf-sandbox/src/maf_sandbox/testing.py)), `exec` (`:185`), `run_code` (`:197`), `stat_file` (`:248`), `read_file` (`:259`), `remove` (`:277`), `reclaim` (`:309`) and `list_dir` (`:327`). Storage is bytes, keyed by normalised absolute guest paths, so it can stand in for a real pull surface rather than only for a text-only one; `seed_files` plants regular content, and `EntryKind.SYMLINK`, `EntryKind.DIRECTORY` or any other kind plants an entry with no content. All four read methods confine `path` to the `working_directory` a call names and run the shared `refuse_symlinked_ancestors` check over the components, the same rule a real backend enforces against its own guest filesystem. `read_file` serves only `EntryKind.FILE` and **refuses** rather than truncates a file over `max_bytes`.
