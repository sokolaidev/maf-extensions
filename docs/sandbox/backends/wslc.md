...
| Which principal a file-plane call acts as | open — unmeasured here. `reclaim` and `remove` are `rm` over this backend's own `exec`, the shape that on [`docker`](docker.md) ran as the guest and could not empty what the host had written; every image this repository runs is root's, which is what keeps it invisible | [#680](https://github.com/sokolaidev/maf-extensions/issues/680) (open) |
