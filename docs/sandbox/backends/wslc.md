...
| Which principal a file-plane call acts as | open — unmeasured here. `write_file` is a tar into `wslc container cp`, so the CLI performs the write rather than the guest, while `exec` names no user at all; `reclaim` is `rm -rf` over that `exec`. `remove` is not served at all, so `reclaim` is the only member with the split — the shape [`docker`](docker.md) had before [#684](https://github.com/sokolaidev/maf-extensions/pull/684) fixed it there. Every image this repository runs is root's, which is what keeps it invisible | [#695](https://github.com/sokolaidev/maf-extensions/issues/695) (open) |
