...
`egress_allow` accepts host strings and `EgressRule` values. A host string allows all methods for that host. A rule can restrict methods or request an attached-authority header.
...
Host matching is case-insensitive. Equivalent entries collapse while retaining the first spelling and method order. Conflicting rules for one host are refused, including a host-wide rule paired with a narrower method or authority rule.
...
| Docker | `CLOSED` through no-network mode; allowlisting through an explicitly configured CONNECT proxy and isolated workload network |
| WSLC | No-network mode or the configured CONNECT proxy; engine-specific limits apply |
...
The proxy permits configured CONNECT destinations and ports, with port 443 as the default. It resolves names on its outbound side and rejects non-global addresses, including private, loopback and metadata addresses. It is recreated on acquisition rather than adopted from an earlier host process.
...
No real backend currently advertises `EGRESS_METHODS`, so the example is refused during preflight on the shipped backends. A CONNECT proxy cannot inspect the method inside an opaque TLS connection. ACAS and Hyperlight also withhold this capability.
...
| Method rules and capability admission | Core implemented; real backend adoption remains open | [#377](https://github.com/sokolaidev/maf-extensions/issues/377) (open) |
