...
| A call's directory can be removed by whoever will be asked to remove it | partial — `write_file` creates it with the backend's own authority while `exec` runs as the image's user, so on a non-root image the removal refused and every call left its files behind. The backend raises authority for a removal on docker, but only over a path with no component the guest could have swapped; the protocol states that reach rule and no more, so an image granting write above `work_dir` still leaks. The other two backends are settled too: `wslc` raises authority and `acas` moves `reclaim` to the data plane's `delete_file`, which already acts as the host | [#680](https://github.com/sokolaidev/maf-extensions/issues/680), docker in [#684](https://github.com/sokolaidev/maf-extensions/pull/684), reach rule in [#686](https://github.com/sokolaidev/maf-extensions/pull/686), wslc in [#706](https://github.com/sokolaidev/maf-extensions/pull/706), acas in [#707](https://github.com/sokolaidev/maf-extensions/pull/707), tracking in [#695](https://github.com/sokolaidev/maf-extensions/issues/695) |
