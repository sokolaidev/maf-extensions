...
| A call's directory can be removed by whoever will be asked to remove it | shipped — `write_file` used to create it with the backend's own authority while `exec` ran as the image's user, so on a non-root image the removal refused and every call left its files behind. Docker's `write_file` now carries the image user's uid/gid on its tar entries (and stamps the call-directory parents explicitly, since implicit intermediates land root-owned), so the call directory is the guest's and the guest's own `rm` can take it. The reach rule still bounds the removal's authority, and since the write half landed that bound costs nothing on the images this suite runs — the guest-owned call directory is one the guest can empty. One case is not covered and is not claimed to be: an image that names a user but answers no identity probe falls back to root's `0:0`, so its call directory is root-owned and a guest-authority removal still cannot take it. That is the pre-#680 behaviour for an image that positively identifies nothing, logged at acquire and tracked on #680 rather than fixed here. The other two backends are settled too: `wslc` raises authority and `acas` moves `reclaim` to the data plane's `delete_file`, which already acts as the host | [#680](https://github.com/sokolaidev/maf-extensions/issues/680), docker's removal authority in [#684](https://github.com/sokolaidev/maf-extensions/pull/684), reach rule in [#686](https://github.com/sokolaidev/maf-extensions/pull/686), wslc in [#706](https://github.com/sokolaidev/maf-extensions/pull/706), acas in [#707](https://github.com/sokolaidev/maf-extensions/pull/707), tracking in [#695](https://github.com/sokolaidev/maf-extensions/issues/695) |
