...
**A backend says the disposal failed by returning, not by raising.** `dispose` is contractually best-effort and never raises, which is what makes it safe to call from a `finally` — and is also why reading only the raise made the refusal above unreachable for every compliant backend: each swallowed its delete error, said nothing, and was read as having disposed. `SandboxBackend.dispose` returns a `DisposalFailure` when a sandbox may still be there, or `None`. It is **a code and a detail**, not a sentence: `DisposalCode` is a closed set — `unreachable`, `timeout`, `refused`, `unlisted`, `unknown` — so a caller deciding whether to retry, alert or escalate branches on a value this package keeps stable rather than matching on a backend's prose. `unknown` is always available, so a backend never has to invent a more precise code to stay in the vocabulary, and several failures fold to the most actionable code with every detail kept. A backend still answering with a bare string is read as `unknown` — the shipped three do until they can import the class from a published core, and that spelling goes away when they have. `None` also covers a backend with no way to check, so the conflation is with success rather than with failure: refusing every key served by a backend that cannot answer would be the wrong direction to fail in. **The refusal carries the code, and quotes it; it does not carry the detail.** A host reading `SandboxUnclean` branches on `SandboxUnclean.code` — the value, not a substring of the message, because a code a caller has to parse back out of prose is not a closed set in any sense that helps. The sentence names it too, for whoever reads an alert. The detail stays beside it in the router's log: a backend's sentence can carry an endpoint, a subscription id or a raw response body, and `acquire` reaches any host directly, not only `sandboxed_tool`, which sanitizes. The model reads a fixed line that names no backend, id or endpoint.
...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` ships; no sample wires it, and neither packaged kind passes it through, so the one call-lifetime decision a host must make has no worked example | [#520](https://github.com/sokolaidev/maf-extensions/issues/520) |
...
| A backend reports a delete that failed without raising, so the refusal fires on a silent failure | partial — `SandboxBackend.dispose` returns a `DisposalFailure` (a `DisposalCode` to branch on, a detail that stays in the log) and `_dispose_each` reads it beside the raise. `dispose_scope` still reads only the raise, so a silently-failed conversation delete reopens every key it had refused; the backends also still answer `dispose` with a sentence, read as `unknown`, until they can import the class from a published core | [#641](https://github.com/sokolaidev/maf-extensions/issues/641), the rest in [#681](https://github.com/sokolaidev/maf-extensions/pull/681) |
