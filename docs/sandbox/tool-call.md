...
| A host learns of a reclaim that did not happen | partial — `on_failure` and `timeout` on `ReclaimConfig` ship as defaults on `SandboxRouter(reclaim=ReclaimConfig(...))`, with `on_reclaim_failure` and `reclaim_timeout` as per-tool overrides on `sandboxed_tool`. It runs **after** the framework has already acted, and `ReclaimFailure.disposal` reports which of `disposed`, `failed` or `kept` it did, so the callback is where a host logs, alerts or counts and not where it decides: safety is the router's, and `ReclaimConfig(failed_reclaim_policy=FailedReclaimPolicy.KEEP)` is the only opt-down. Its own exception is logged and swallowed. What is missing is a worked policy over that field — what to page on when a disposal did not land and the router is refusing the conversation until one does: proposed as an in-process sample over `maf_sandbox.testing`, whose removal can be made to fail on demand, and held on sequencing | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the router default and policy in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
