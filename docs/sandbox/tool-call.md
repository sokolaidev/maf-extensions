...
| The framework disposes a sandbox it could not clean — a failed reclaim, or a stop that did not reach the process group — and the host loosens that on the router | open — not shipped: `on_reclaim_failure=None` logs and carries on, the only switch is on `sandboxed_tool`, and neither packaged kind passes it through | [#617](https://github.com/sokolaidev/maf-extensions/issues/617) (open) |
