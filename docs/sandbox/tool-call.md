...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` and `reclaim_timeout` ship with defaults on `SandboxRouter(reclaim=ReclaimConfig(...))` and per-tool override on `sandboxed_tool`. It runs **after** the framework has already acted, and `ReclaimFailure.disposal` reports which of `disposed`, `failed` or `kept` it did, so the callback is where a host logs, alerts or counts and not where it decides: safety is the router's, and `ReclaimConfig(failed_reclaim_policy=FailedReclaimPolicy.KEEP)` is the only opt-down. Its own exception is logged and swallowed. What is missing is a worked policy over that field — what to page on when a disposal did not land and the router is refusing the conversation until one does. [#520](https://github.com/sokolaidev/maf-extensions/issues/520) proposes it as an in-process sample over `maf_sandbox.testing`, whose removal can be made to fail on demand, and holds it on sequencing; its own text predates [#617](https://github.com/sokolaidev/maf-extensions/issues/617) moving the disposal ahead of the callback | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the router default and policy in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
| A call's directory can be removed by whoever will be asked to remove it | partial — `write_file` creates it with the backend's own authority while `exec` runs as the image's user, so on a non-root image the removal refused and every call left its files behind. [#684](https://github.com/sokolaidev/maf-extensions/pull/684) settled it in the backend rather than in every kind that writes a file: `maf-sandbox-docker` raises authority for a removal, but only over a path with no component the guest could have swapped. The protocol states that reach rule and no more, so an image granting write above `work_dir` still leaks. The other two backends are settled too: `wslc` raises authority ([#706](https://github.com/sokolaidev/maf-extensions/pull/706)) and `acas` moves `reclaim` to the data plane's `delete_file`, which already acts as the host ([#707](https://github.com/sokolaidev/maf-extensions/pull/707)) | [#680](https://github.com/sokolaidev/maf-extensions/issues/680), the reach rule in [#686](https://github.com/sokolaidev/maf-extensions/pull/686), the other two backends in [#695](https://github.com/sokolaidev/maf-extensions/issues/695) |
