...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` ships, and sample 07 wires a handler and has its live check refuse a run that fires it. Two gaps behind that: no sample can make a removal *fail* on a real backend, and neither packaged kind passes the callback through, so a host on `codeact` or `bicep` cannot wire one at all | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the pass-through in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
