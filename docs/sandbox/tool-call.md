...
| A backend reports a delete that failed without raising, so the refusal fires on a silent failure | shipped — `SandboxBackend.dispose` returns the reason a sandbox may still be there, and `_dispose_each` reads it beside the raise; all four in-tree backends report | [#641](https://github.com/sokolaidev/maf-extensions/issues/641) |
