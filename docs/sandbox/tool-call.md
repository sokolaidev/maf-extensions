...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` ships and no sample wires one, deliberately: it is a notification rather than a veto, since the body has returned by the time it runs and a handler that raises is contained. What a host does with it — count it, page when `disposal` is `"failed"` and the router is refusing the conversation — is operations code rather than a wiring a sample can show. Two gaps behind that: neither packaged kind passes the callback through, so a host on `codeact` or `bicep` cannot wire one at all; and the failure a sample would have staged is the one [#684](https://github.com/sokolaidev/maf-extensions/pull/684) fixed, so staging one now needs an image that hands the guest a directory above `work_dir` | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the pass-through in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
