...
| A backend reports a delete that failed without raising, so the refusal fires on a silent failure | partial — `dispose` returns a `DisposalFailure` (a `DisposalCode` to branch on, a detail that stays in the log) and the router reads it beside the raise. `dispose_scope` still reads only the raise, so a silently-failed conversation delete reopens every key it had refused; the backends answer with a sentence, read as `unknown`, until the class is published | [#641](https://github.com/sokolaidev/maf-extensions/issues/641), the rest in [#681](https://github.com/sokolaidev/maf-extensions/pull/681) |
