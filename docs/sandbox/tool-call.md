...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` ships and no sample wires one. It runs **after** the framework has already acted, and `ReclaimFailure.disposal` reports which of `disposed`, `failed` or `kept` it did, so the callback is where a host logs, alerts or counts and not where it decides: safety is the router's, and `SandboxRouter(keep_unclean=True)` is the only opt-down. Its own exception is logged and swallowed. What is missing is a worked policy over that field — what to page on when a disposal did not land and the router is refusing the conversation until one does. [#520](https://github.com/sokolaidev/maf-extensions/issues/520) proposes it as an in-process sample over `maf_sandbox.testing`, whose removal can be made to fail on demand, and holds it on sequencing; its own text predates [#617](https://github.com/sokolaidev/maf-extensions/issues/617) moving the disposal ahead of the callback. Second gap: neither packaged kind passes the callback through, so a host on `codeact` or `bicep` cannot wire one at all | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the pass-through in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
