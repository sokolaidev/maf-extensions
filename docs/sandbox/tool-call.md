...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` ships and is sampled twice: sample 07 wires a handler and its live check refuses a run that fires one, and sample 12's act 5 stages a removal that refuses and reads all three `DisposalOutcome`s off the run, the key the router goes on refusing included. No real backend can be told to refuse a removal, so that act runs in-process and says so. What is left is library-side: neither packaged kind passes the callback through, so a host on `codeact` or `bicep` cannot wire one at all | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the pass-through in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
