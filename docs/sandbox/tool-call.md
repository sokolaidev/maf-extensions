...
| A call's directory can be removed by whoever will be asked to remove it | open — `write_file` creates it as the backend's writer while the removal runs as the guest's `exec` user, so on a non-root image it can never be reclaimed and every call leaves its files behind. A kind could work around it by making the directory from the guest first, and none does: that puts an OS command and a POSIX assumption in a layer [#585](https://github.com/sokolaidev/maf-extensions/issues/585) is arguing them out of. The protocol says nothing about ownership and no backend promises anything | [#680](https://github.com/sokolaidev/maf-extensions/issues/680) |
