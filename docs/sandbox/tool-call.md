...
| A host learns of a reclaim that did not happen | partial — `on_reclaim_failure` ships and no sample wires one. What is missing is a worked *policy*: the callback is a notification rather than a veto, so the interesting part is what a host does with it — what it logs, when it escalates to a disposal that also ends sibling calls, and that a handler must never raise. [#520](https://github.com/sokolaidev/maf-extensions/issues/520) proposes that as an in-process sample over `maf_sandbox.testing`, whose removal can be made to fail on demand, and holds it back on sequencing rather than on difficulty. Second gap: neither packaged kind passes the callback through, so a host on `codeact` or `bicep` cannot wire one at all | [#520](https://github.com/sokolaidev/maf-extensions/issues/520), the pass-through in [#677](https://github.com/sokolaidev/maf-extensions/issues/677) |
