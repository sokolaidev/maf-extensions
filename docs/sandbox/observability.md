...
| An OpenTelemetry recorder, under the app's providers or a security pipeline's own | partial — `maf-sandbox-otel` implements it: it registers on the router and the host-tool registry and turns each event into a log record, a span where the event carries a duration, and a metric where it counts, under the app's providers or a pipeline's own. Core still cannot host it, since its protocol modules are standard library only. What is not covered is what core does not emit — the scope purge in the row below, and the proxy's own lines in the row after it | [#904](https://github.com/sokolaidev/maf-extensions/issues/904) (open); the package is [#907](https://github.com/sokolaidev/maf-extensions/pull/907) |
