...
|---|---|---|
| A `SandboxObserver` a host registers, and frozen events in core's own vocabulary | shipped — six events, registered on `SandboxRouter` and `HostToolRegistry`, passed to `collect_outputs`; refusals, cancellations and served calls alike | [#906](https://github.com/sokolaidev/maf-extensions/pull/906) (merged), under [#904](https://github.com/sokolaidev/maf-extensions/issues/904) (open) |
