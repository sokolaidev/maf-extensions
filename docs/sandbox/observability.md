...
| `EgressObserved` | A backend implementing `ObservesEgress`, before each removal that would take its enforcer's record with it | What the guest actually reached: one `EgressDecision` per `CONNECT` the enforcer answered — `ALLOW`, `DENY`, `DENY-NONGLOBAL` or `UNREACHABLE`, with the host and port it was asked for. The read is bounded, so `truncated` says when it was cut, and `unreadable` names a window the backend could not account for |
