...
| `EgressObserved` | A backend implementing `ObservesEgress`, before each removal that would take its enforcer's record with it — **except one it cannot attribute**, since a scope purge is addressed by a conversation and this event needs a key | Every `CONNECT` the guest **attempted** and how the enforcer answered it: one `EgressDecision` each — `ALLOW`, `DENY`, `DENY-NONGLOBAL` or `UNREACHABLE`, with the host and port it asked for. Only `ALLOW` opened a tunnel, so a reader counting what was *reached* filters on the verb. The read is bounded, so `truncated` says when it was cut, and `unreadable` names a window the backend could not account for |
...
That difference is why silence is not a reading. A key with no `EgressObserved` record was either watched and quiet or never watched at all, and the two are the opposite conclusions; `BackendDeclarations.observes_egress` rides on the acquire record so a reader can tell which. [`network.md`](network.md) carries why a host two conversations share is worth seeing at all.
