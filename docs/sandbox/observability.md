...
**What a call was fed is folded once, and its absence is an answer.** Every read is a `StoreFileRead`; `ToolCallEnded.fed` is the fold across the ones that carried text — `weakest_integrity` over their labels, with how many reads it folds — so a recorder asking *was this call fed anything the host did not establish* reads one field instead of re-deriving the ordering and the empty case from the per-read records. A call that read nothing carries no fold at all rather than the `trusted` an empty listing folds to: that answer is honest about a result deriving from no file and would read here as a call fed trusted content. And it describes the call's **inputs**, never its result — a kind that reads a file and answers a fixed sentence is fed exactly what one quoting the bytes back is, so what a result derives from stays the kind's declaration.
