...
**The key is what joins them, and the seven do not all carry one the same way.** Four shapes, because four things are true:
...
**`call` is the second join column, and it is the one the key cannot be.** A key names a sandbox and a conversation; at the default `conversation` scope it names no call, so two calls in flight on one thread are one key and their records interleave. Every event carries `call` — the id of the tool call it came from — and on `ToolCallEnded` it is the only field on the seam that is never `None`, because that record is where a call's other events join. On the other five it is absent for what genuinely happened outside a call: an acquire a direct consumer of the router asked for, a disposal from a scope purge or a framework reclaim, a collection a kind ran outside a tool body.
...
**The egress proxy's own decisions.** The docker and wslc proxy prints `ALLOW`, `DENY`, `DENY-NONGLOBAL` and `UNREACHABLE` inside its own container; the backend reads that stream once, at acquire, to wait for the `listening` marker, and the proxy is recreated per acquire so the lines die with the container. The lines carry no sandbox key, though the container they are printed in does — both backends label the proxy with the key that asked for it — so what is missing is a read taken before the container goes away rather than a way to attribute one. A backend that enforces egress in the service — `acas` — exposes the deny reason only inside the guest and has no such stream to read at all. So `SandboxAcquired` records the mode and the allowlist a sandbox was **served** under, which is what a spec asked for and not what a guest then reached. Routing those lines to a record is [#948](https://github.com/sokolaidev/maf-extensions/issues/948), and [`network.md`](network.md) carries why a host two conversations share is worth seeing at all.
...
**A collection, and a host-tool run's key, on the kind that has them.** `collect_outputs` takes the observer and the key as arguments rather than reaching for them, and records nothing when it is given neither — so the event is defined and no shipped kind produces one. codeact is the only kind that lands outputs or calls host tools, and it passes neither the observer on its collection nor the key on its `HostToolRun`: files **out** are recorded by nothing today, and a host-tool record is keyed on its `run_id` alone. Both values are already in scope at both call sites, which is [#949](https://github.com/sokolaidev/maf-extensions/issues/949).
...
|---|---|---|
| A `SandboxObserver` a host registers, and frozen events in core's own vocabulary | shipped — seven events, registered on `SandboxRouter` and `HostToolRegistry`, passed to `collect_outputs`; refusals, cancellations and served calls alike | [#906](https://github.com/sokolaidev/maf-extensions/pull/906) (merged), under [#904](https://github.com/sokolaidev/maf-extensions/issues/904) (open) |
...
| The proxy's `ALLOW`/`DENY` lines reach a record, keyed to the sandbox that caused them | open — the lines die with the proxy container, though the container carries the key that would join them; for a service-enforced backend they never leave the guest at all | [#948](https://github.com/sokolaidev/maf-extensions/issues/948) (open) |
| A host-tool record carries the key of the sandbox its run belongs to | partial — `HostToolRun(key=…)` accepts one and every record carries it; the shipped `execute_code` transport does not pass one yet | [#949](https://github.com/sokolaidev/maf-extensions/issues/949) (open) |
| A collection's record is joined to the conversation that produced it | partial — `collect_outputs(observer=…, key=…)` takes both, and `SandboxToolSession.observer` is where a kind reads the first; codeact passes neither, and it is the only kind that lands anything, so no collection is recorded at all today | [#949](https://github.com/sokolaidev/maf-extensions/issues/949) (open) |
