...
- `HostToolCalled`, `StoreFileRead` and `OutputsCollected` type it `SandboxKey | None`, because each has a reachable case with no sandbox behind it: a transport constructed without a key, a read on a session that never acquired, a collection a kind did not pass one for.
