...
**Measured, but not through a live agent:** the third row. Reaching an untrusted conversation needs a *visible* untrusted item, and hiding is precisely what prevents one, so the context label was set directly and the real middleware and real tool driven around it. The attempt to reach it honestly — giving the agent `inspect_variable` so it would taint the conversation by reading an innocuous variable, then pass the payload it had never read — **failed for an unrelated reason worth recording**: `inspect_variable` logged *"No middleware context found, using global variable store"* and then *"not found"*. The middleware publishes itself in a `threading.local`, and a synchronous tool body dispatched to a worker thread does not see it. The same thread boundary is why `values_holding_hidden_content` documents that it answers empty for a synchronous body.
