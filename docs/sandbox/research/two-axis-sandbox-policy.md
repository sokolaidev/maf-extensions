...
class Isolation(StrEnum):                     # str-valued: serializes and compares as its value,
    ...
    PROCESS = "os_process"                    # separate OS process: kernel-enforced address space,
                                              # shared kernel and filesystem, no namespaces
...
- `none` is the bottom rung, named for what it provides rather than for where it runs. It was `process` until [#262](https://github.com/sokolaidev/maf-extensions/issues/262): that spelling read as a real boundary and meant the absence of one. The `PROCESS` *name* has since been taken back for the genuine separate-OS-process rung two ranks above, but the string `"process"` has not and never will be — it raises `ValueError` in every release from the rename onward. The name is resolved where the code is written, so reusing it is a decision someone makes; the string is resolved at run time out of configuration nobody re-reads, so reusing it would have been a re-ranking nobody saw. The two rungs never share a value, and that is what keeps a declaration written against the old meaning a refusal rather than a promotion.
- `runtime` sits between `none` and `container`, and exists to draw the line between *no boundary at all* and *a software boundary*: a literal in-process function call (the testing fake) is `none`; a sandboxing language runtime — Monty's restricted interpreter, a Wasmtime-class WASM runtime's software fault isolation with capability-based imports — is `runtime`. The boundary is real (OS access rejected by construction, linear-memory confinement), but it is enforced by software in the host process's own address space: an escape is a runtime bug and lands *inside the host process*, beside its memory and credentials, with no second privilege domain in the way — which is why the rung sits below `container`, whose enforcement lives in the kernel, an independent domain. The ordering ranks trust bases, not implementations — it is not a claim that every kernel beats every verified SFI runtime — and the capability axis carries the rest of the honesty: Monty declares no filesystem and no network capabilities at all, which the ladder alone could never express.
- `os_process` sits between `runtime` and `container`, and is the rung a backend declares when it runs the workload in a separate OS process with nothing else in the way: a kernel-enforced address space, but a shared kernel, a shared filesystem and no namespaces. It is above `runtime` because the enforcement moves out of the host's own address space and into the kernel, a second privilege domain — an escape no longer lands beside the host's memory and credentials. It is below `container` because a container *is* this plus namespaces and cgroups, so ranking it lower is the clean generalization rather than a judgement call. No backend in this repository provides it, and the rung exists anyway: without it a backend that ran untrusted code in a subprocess would have to understate itself as `runtime` or overstate itself as `container`, and the ladder's whole value is that neither is available.
...
class Capability(StrEnum):
    ...
    HOST_TOOLS = "host_tools" # call host-registered functions from inside the sandbox
...
> **Status:** the first two bullets shipped and the third did not, and nothing on this page said so until now. A backend declares its capabilities and a spec declares `requires`, and `ensure_can_serve` refuses the difference — both released, both still true. **Selection generalized late, and as an opt-in.** For most of this package's life `SandboxRouter._resolve` ran once in `__init__`, taking `_backends[0]` or the `selected=` name, and every later call used that one backend — so a spec it could not meet was *refused*, with a registered backend that could have served it sitting unused. `Selection.PER_SPEC` ([#328](https://github.com/sokolaidev/maf-extensions/issues/328)) is that bullet as built, and it differs from the bullet as written in the one way worth recording here: it is **off by default**. Routing can only ever serve a spec that is refused today, so what it changes is that a refusal becomes a running sandbox — and on a remote backend a running sandbox has a price, which is a trade a host takes rather than inherits. That comparison is against an *unpinned* router with the same registration order: a host migrating off `selected=` must drop the pin, since the two are refused together, and routing then begins at the first registered backend rather than the pinned one. The rules as shipped are [`../capabilities.md`](../capabilities.md) § "A match by default, and a search when a host asks for one". `samples/11_router_two_backends` is where the *fixed* selection can be watched refusing, with a capable backend registered and unused; the act that watches a spec route has not landed with it.
...
> **Status:** the backend-agnostic contract landed via [#133](https://github.com/sokolaidev/maf-extensions/issues/133) part A — `HostToolRegistry`, the `@sandbox_tool` decorator with a mandatory `identity` leg pulled forward from step 6, the `require_declared` gate at registration and at call time — with the declaration captured at registration and the registry sealed when its aggregate is taken, so the surface a host classified is the surface that serves the calls — a per-run host-tool-call cap, host-side argument validation at the registry's one door, response size caps reusing `TransferLimits`, and `denied_capabilities` / `denied_identities` on the router. The transport an `EXEC` backend can implement honestly (part B) has landed too — `host_tool_calls_over_exec`, request and response files over `EXEC` + `FILES_IN` + `FILES_OUT`, with a generated guest shim that is convenience rather than a control and a supervisor that resolves every request through the same one door. The kind integration (part C) has landed too, and the backends followed in that order rather than ahead of it — the contract, the channel and the kind existed before any backend could use them, and `maf-sandbox-docker` ([#410](https://github.com/sokolaidev/maf-extensions/pull/410)) then `maf-sandbox-acas` ([#417](https://github.com/sokolaidev/maf-extensions/issues/417)) each declared it on its own live measurement that `exec` detaches, which is the whole of what the capability adds to a backend that already serves `EXEC` and the pull surface. One sentence to read before registering anything, because a declaration reads like a control and is not one: **`Identity.APP` is not the safe option, only the declared one** — it is the application's full authority, and the only real bounds on it are the emptiness of the registry and the host-tool-call cap. `Identity.USER` is served where a host mints it: `HostToolRegistry(mint_user_identity=…)` supplies the run's authority and it reaches the body as `user_identity`, while a registry without one registers such a tool and refuses its call. Registering one raises the whole `execute_code` surface to approval-gated either way. Of the three prerequisites this page once named, only per-run minting ever bounded a host-tool call, whose body runs host-side; audience ⊆ egress and the ephemeral `exec` env channel are C′'s in-guest half and moved to [#757](https://github.com/sokolaidev/maf-extensions/issues/757).

Calling host functions from inside a sandbox is the CodeAct pattern's differentiator and the one capability where trust crosses *outward* — the function body runs in the host process with the host's privileges, driven by model-written code, and each host-tool call bypasses whatever middleware the host runs. It ships as six layers:

1. **Nothing is callable by default.** The sandbox tool registry starts empty; every function reachable from inside is one a developer explicitly registered.
2. **Registering emits a one-time, suppressible warning** (the experimental-warning shape) naming the property that surprises people: host-tool calls bypass the middleware chain and the boundary sees only the aggregate result.
3. **A role-explicit decorator carries each tool's information-flow declarations.** A called function is a **source** (output brings external data in), a **sink** (conversation-derived data flows out or drives an effect), **both**, or **neither** (pure computation). The decorator makes the developer answer every leg explicitly, with no defaults — `@sandbox_tool(source=..., sink=..., identity=...)`, each leg's `None` a considered "not that role" — and the values are the host's own vocabulary (`agent_framework.security`-shaped constants for MAF hosts, verbatim passthrough otherwise).
...
- **B. Host-tool calls acting as the user — recommended for user authority.** The function body runs host-side and reaches the host's on-behalf-of plumbing; the credential never enters the sandbox, only results do. The decorator's `identity` leg (`Identity.APP | Identity.USER | None`) makes it declarable; any `Identity.USER` tool raises the aggregate (user-confidential sources, approval-gated call).
...
| Hyperlight (`maf-sandbox-hyperlight`, [research record](hyperlight.md)) | backend *family* — declarations derive from the configured guest | `microvm`, measured against the standard on (wasm × WHP) | `RUN_CODE, SNAPSHOT` (+file capabilities and `HOST_TOOLS` pending separate work) | `allowlist` — the earlier `closed` reading of this row was wrong: `allowed_domains` is native per-entry enforcement |
...
`requires` uses `EXEC` — the ACA Sandboxes road: `write_file` the program, `exec` the interpreter. The same kind could later ship a `RUN_CODE` variant served by an embedded-interpreter backend; *the spec is where that choice lives*, and the router picks whichever registered backend satisfies it — which rests on the generalization the Axis 2 status note above records, and which now exists as the opt-in `Selection.PER_SPEC` ([#328](https://github.com/sokolaidev/maf-extensions/issues/328)). It does not follow that this example works: whether a second spec is even the right shape for that variant, against a disjunction in the matcher, is [#425](https://github.com/sokolaidev/maf-extensions/issues/425) and is open — and until it is answered `codeact_sandbox_spec` requires `EXEC` flatly, so a `RUN_CODE`-only backend is never a *candidate* for it however willing the router is to route.
