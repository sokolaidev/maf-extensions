...
The prepared-image workflow lives in the [image usage guide](../../../images/terraform-sandbox/USAGE.md#build-a-prepared-image), and its full request contract in the [image guide](../../../images/terraform-sandbox/README.md#approved-dependency-preparation). The open follow-up [#1249](https://github.com/sokolaidev/maf-extensions/issues/1249) covers approved artifacts and request paths; runtime validation continues to use closed egress.
...
| The same build with preparation as its first stage | 3 minutes 10 seconds from an empty cache, with the same 104-second preparation; 67 seconds when only the stages after preparation changed, since the 141 probes run again; under a second when no stage changed |
...
The registry is the other difference. `registry.opentofu.org` serves the same provider protocol as `registry.terraform.io` and decides the artifact a pin takes. The six providers the OpenTofu organization rebuilds are served from `github.com/opentofu/`, with digests Terraform's manifests do not carry; a release its vendor publishes can be the same asset, and `azapi` 2.12.0 is pinned to the same bytes here and in both AVM manifests. It answers a request that arrives without a `User-Agent` of its own with 403, which the generator now avoids by naming itself. Registry modules stay Terraform-only, so an OpenTofu policy pins providers alone.
...
The decided workload contract is maintained in the [Terraform and OpenTofu kind guide](../kinds/terraform.md), image contents and preparation rules in the [image guide](../../../images/terraform-sandbox/README.md), and build, preparation and runnable-example instructions in the [image usage guide](../../../images/terraform-sandbox/USAGE.md). This record keeps the evidence and unresolved design questions that led to those decisions without duplicating their full operational instructions.
