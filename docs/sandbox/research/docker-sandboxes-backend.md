...
- **Stored secrets follow the sandbox's egress.** The test host has a global `github` service secret. Every sandbox reports `SBX_CRED_GITHUB_MODE=apikey`, meaning the proxy will inject that token into the guest's requests to GitHub's domains. Under `CLOSED` nothing reaches them. Under an allowlist naming a GitHub host, the guest acts with the host user's GitHub credential. The backend must read `sbx secret ls` at acquire and refuse any allowlist that overlaps a stored secret's domains.
