...
Built as [`maf-sandbox-docker-sbx`](../../../packages/maf-sandbox-docker-sbx/README.md), described in [the backend page](../backends/docker-sbx.md). The build departed from this section in three places. The workspace is bound at the storage base's parent, because `pwd` must name the base. Removals run in the guest, because a guest keeps seeing a name for seconds after a host-side delete. `HOST_TOOLS` is withheld until the transport is measured against the 30-second auto-stop.
