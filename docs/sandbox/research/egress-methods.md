...
`GET`-only against an API host removes request bodies and uploads from the channel. It **narrows** the sink; it does not **close** it — a `GET` exfiltrates through the URL, the query string and the headers just as well. So the honest claim is *narrowing*, and every place the axis surfaces has to say so, or `GET`-only will be read as no-exfiltration by the first host that wants it to mean that. This is the single framing constraint the whole design bends around: literal HTTP verbs, no `read_only=True` sugar, and a docstring that states the limit where a caller reads it.
...
- **Token shape** — each method is a non-empty token free of whitespace and commas, and there is **no HTTP-verb whitelist**: WebDAV and custom verbs are real, and whether a *given* verb is enforceable is the backend's concern, not the vocabulary's. Same shape as the hostname check, which validates form and not membership in a list of known hosts.
...
- **Duplicate host** — a host may appear **once**. A bare string and an `EgressRule` for the same host, or two `EgressRule`s for it, is a `ValueError`: which one wins is ambiguous, and the same "one entry per host" instinct the plain allowlist already carries.
...
- **The backends** — those that read `egress_allow` (docker's egress-id fold, ACAS's policy builder) now see `str | EgressRule`, and their sandbox-key fold must include the methods so a `GET`-only and an all-methods sandbox for the same host do not collide into one warm container. None of them **declares** the capability: the CONNECT-tunnel pair cannot, ACAS could in principle but the fold is the only change until it does, and the first real declarer is Hyperlight.
