...
**Where it is established.** By the backend, on the cold path of `acquire`, where docker and acas already probe; wslc gains a probe. The fact belongs to the sandbox it was measured on. A backend that shares it across sandboxes keys it by an immutable artefact identity, a content digest or a disk-image id, and never by a name: a reference like `python:3.13` can be repointed between two creates, and a memo keyed by it would hand the second container the first image's principal. docker already keeps its facts per container and forgets them when the container goes; acas's memo moves from the spec's image name to the disk-image id it resolves, since a name in the group can be re-imported too.
...
**The parked half.** `EGRESS_HEADER` for run credentials on acas needs a secret per sandbox, created with the sandbox and deleted with it, and never a group-wide one: a rule on every sandbox referencing one secret would hand sandbox A's requests the token sandbox B wrote last, across conversations. Within one sandbox, two concurrent runs of one conversation still contend for the one value, so the channel serialises the write per sandbox or is served at `IsolationScope.CALL` only. Measurement M4 decides it, and it measures under two concurrent sandboxes and two concurrent runs, not one: whether the proxy reads the secret's current value at request time on a warm sandbox, and whether each sandbox's requests carry only its own. If both hold, acas declares the channel and the token never enters the guest at all, which is the better cell. If either fails, the channel is servable only at `IsolationScope.CALL`, where the rule and the secret are created per call, and it waits for that.
...
`BackendDeclarations` grows two fields, `attached_identity` and `credential_channels`, and stays one object read with one `getattr`. `SandboxSpec` grows four, `confined_to_call_path` and `min_cleanup` in wave 1 and `max_identity_scope` and `run_credential` in wave 2, appended after `isolation_scope` for the positional reason every field there is appended.
...
- **Serialise calls** ([#476](https://github.com/sokolaidev/maf-extensions/issues/476)). It counts them, so a cleanup above `RECLAIM` waits for the last call out; nothing queues. Declaring `CALL` on a backend stays [#436](https://github.com/sokolaidev/maf-extensions/issues/436)'s, and Pillar E's bound is why it matters more after this lands.
...
| M4 | On acas, whether a `secret set` after create changes the header the proxy injects on a warm sandbox's next request; and, with two sandboxes on per-sandbox secrets and two concurrent runs in one of them, whether each request carries only its own sandbox's and run's value | Whether `EGRESS_HEADER` run credentials ship on acas, and at which scope |
...
| M6 | On acas, the latency of a create from a snapshot against a create from the shipped disk images; whether the snapshot brings back a booted VM with its processes or only its disk; the snapshot quota and cost per sandbox; and whether a snapshot goes with its sandbox or the backend must delete it | Whether acas declares `SNAPSHOT` and builds `reset`, and so whether the `RESET` rung exists on any shipped backend |

### Wave 1 — the guest principal, the cleanup ladder, and the rung below the key

One core `feat!`: `GuestPrincipal`, `Sandbox.guest_principal`, `refuse_capabilities_the_guest_cannot_back`; `Cleanup` and `CLEANUP_RANK`, `SandboxSpec.confined_to_call_path`, `SandboxSpec.min_cleanup`, `SandboxRouter(min_cleanup=...)`, `effective_cleanup`, the in-flight count; `Capability.RECLAIM`, `Sandbox.reset` behind `Capability.SNAPSHOT`, `SandboxBackend.dispose(key, *, kind=None)`, `SandboxRouter.dispose_kind`, the served-by table; the `reclaim` docstring rewrite; and the conformance additions, `assert_nothing_left_behind` with its fingerprint seam on the subject, the principal probe, the per-kind disposal probe, and the RECLAIM suite moved onto its gate. The fake's repairs land first as their own pull request, and the fake answers the fingerprint from its dict so the ladder is testable without an engine.

Then, after the core release, one pull request per backend, each declaring `RECLAIM`, reporting a principal, accepting `kind` on `dispose`, and calling the shared refusal: docker observing the uid through a tar header where the account files do not answer, which closes #741, and its conformance subject answering the fingerprint from `docker diff` and `docker top`; acas with the ancestor check over its data-plane stat, #710's measured facts in its `reclaim` docstring, its principal memo keyed by disk-image id, and `SNAPSHOT` with `reset` only if M6 says the create from a snapshot is worth it; wslc with a uid probe, the ancestor check asked of the guest, and the principal named in its `remove` refusal, which closes #839's question. Then the kinds: bicep moves its module cache under the call path, declares `confined_to_call_path`, and runs the probe in its own suite on docker; codeact declares nothing and its README states that every call now pays a create where no backend declares `SNAPSHOT`. Samples take the floor bump; sample 13's reuse assertion holds under bicep's claim, and the codeact samples' live checks are re-timed on acas. The tracker rows in `tool-call.md`, `capabilities.md` and the three backend pages flip in the pull requests that deliver them.
...
Gated on M4: `EGRESS_HEADER` run credentials on acas, or the same at `IsolationScope.CALL` only. Gated on M6 going the other way: if a create from a snapshot is not worth it, codeact's warm reuse on acas returns only when the service adds an in-place restore, and the backend page says so. Gated on a backend declaring `CALL`: the docs' recommendation that credential-carrying workloads run there becomes a sample. Gated on #328: narrowing the unclean refusal to a kind.

## Decisions taken, and decisions this asks for

**Taken.** A call leaves nothing behind by default. Warm reuse is earned by a kind's confinement claim over a backend's `RECLAIM`, proved on docker, or by a backend's snapshot reset, and a host may raise the floor above either. This is #792's default, for every image rather than for root's.
