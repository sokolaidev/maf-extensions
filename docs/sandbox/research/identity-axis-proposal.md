...
**The claim is proved where the backend can diff.** Core ships `assert_nothing_left_behind(subject, call)`: fingerprint the sandbox, run the workload's own call and its cleanup, fingerprint again, and fail on any difference outside what the rung permits. A `ConformanceSubject` answers the fingerprint from its engine. docker answers with `docker diff`, which lists every path changed since the container was created, so on the one backend a pull request runs the kind's claim is exercised against the real workload rather than read. The in-process fake answers from its own dict. A backend with no diff primitive skips the probe and the claim stands as a claim there. A kind that declares confinement owes this probe in its own suite, on docker.
...
The cleanup ladder adds no check: `DISPOSE` is always established, so `effective_cleanup` answers rather than refuses. Two consistency refusals join the resolve-time ones: a capability list and an identity declaration that disagree, and a `PER_SANDBOX` or `PER_SCOPE` identity declared without an auto-delete bound. The `Identity.SANDBOX` cross-check needs no new exception, since the spec refuses the combination before the router sees it and the capability match catches the rest.
