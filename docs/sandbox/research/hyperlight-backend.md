...
The first controller-connection-loss probe exposed an error-mapping defect: a failed ledger read escaped as a raw subprocess error even though ownership remained reserved. Recovery now reports `HyperlightPodCleanupPending` for failed ownership lookup and failed termination-receipt persistence. Eight regression cases failed before the fix and passed afterward, retaining the ledger/finalizer until a successful retry. The real connection-loss probe then passed with the corrected controller. The full local gate passed with 11,321 tests and 598 platform/live skips; those skips are not live AKS passes.
