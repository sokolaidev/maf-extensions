...
- **Reading bytes the kind itself consumes** — a SARIF file parsed into diagnostics, a JSON result summarised into the tool result — is a **source**. The question is *integrity*: is what came back trustworthy? Already answered by `sandbox_tool_declarations(source_integrity=...)`. This record said a kind running a guest program answers it by declaring nothing and taking the untrusted default; [#840](https://github.com/sokolaidev/maf-extensions/issues/840) ended that — every shipped kind now declares `untrusted` explicitly, because declaring nothing delegates to tier 3 and the host's `default_integrity`, and neither knows about the code inside the sandbox that emitted the bytes.
