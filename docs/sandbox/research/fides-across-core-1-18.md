...
**Measured in the default configuration, which is one of three outcomes and the only one that refuses.** The same call that ran to completion on 1.17, with the expanded payload in the body's hands, terminates on 1.18 with the body never entered and one `untrusted_arguments` entry in the audit log. The other two are read from the source rather than measured, and both serve the call: with `approval_on_violation=True` the first attempt raises `MiddlewareTermination("Policy approval required")` and an *approved* retry falls through to `call_next`, so the body runs with the payload; with `block_on_violation=False` and no approval mode the violation is logged and the call proceeds. So wiring the middleware is not on its own a refusal — leaving its defaults alone is.
