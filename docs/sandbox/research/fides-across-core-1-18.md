...
| that reference passed to a tool declaring nothing, with the policy middleware wired | expanded, body runs | `untrusted_arguments`, call blocked |
...
What *does* narrow the problem is the policy middleware. From 1.18 the tracker publishes `argument_label` and the policy middleware raises `untrusted_arguments` on it, blocking any call whose arguments resolve to untrusted content unless the tool opted in through `allow_untrusted_tools` or an `accepts_untrusted` property. Measured: the same call that ran to completion on 1.17, with the expanded payload in the body's hands, terminates on 1.18 with the body never entered and one `untrusted_arguments` entry in the audit log. Upstream's own model-facing instructions now say so in as many words — *"Forwarding is allowed only when the destination tool declares `accepts_untrusted=True`; otherwise the call is blocked, audited, or sent for policy approval."*

Neither shipped kind opts in, and neither should: opting in is what would reopen the channel. So on 1.18 with both middleware wired, the chain in [`laundering-into-the-file-store.md`](laundering-into-the-file-store.md) is refused at the step where the model forwards the reference. What it leaves untouched is the configuration these helpers were written for — a host wiring the tracker alone — and every core below 1.18 with both wired. Both are inside the range this suite accepts, so nothing retires.
