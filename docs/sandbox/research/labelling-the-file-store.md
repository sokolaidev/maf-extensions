...
**The name is a second channel, and it is model-authored every time.** `files: list[str]` is a string the model typed, whoever placed the file it points at — and a model can put anything into a name, a payload included. A per-file label is therefore a label on the **bytes** behind a name and never on the name itself, which stays an unlabelled argument under tier 3 and resolves to the untrusted floor like every other plain `str`. Two rules this suite already has keep it out of the result. A name absent from the host's listing is refused before it reaches any command, so an invented one has nowhere to go; and a refusal names the rejected value by its *position* — `files[1]` — through `echoed_name`, never by echoing it ([#818](https://github.com/sokolaidev/maf-extensions/pull/818)). A name that *is* in the listing carries host-authored bytes, but the model's choice of it is still a presence bit under rule 5. So a kind that renders a per-file verdict must render it by position, and the `Content` carrier below must hold the file's bytes and its label, not the string the model passed.
