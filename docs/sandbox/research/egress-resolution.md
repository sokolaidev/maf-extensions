...
> The proposal that argued egress out of a property to match and into a mode to run in — one ordered axis, the workload declaring the mode and the backend the set it enforces, refuse never degrade — tracked by the umbrella [#265](https://github.com/sokolaidev/maf-extensions/issues/265). It is kept in the tense it was written, as the record of the argument rather than a description of what shipped. The decided content now lives in [`../network.md`](../network.md) and [`../policy-isolation.md`](../policy-isolation.md).
