...
The implementation now records the mode and normalized host set with the held instance and refuses a mismatch on a usable instance before resume. It retains the original holder and cleanup ownership. A registry-key guard serializes acquisition across event loops; offline regressions overlap two threads and loops during creation and exercise waiter and owner cancellation. The refusal is `AcasEgressPolicyConflict`, a `SandboxEgressNotEnforced` subclass. A caller must successfully dispose the kind or choose another key before changing policy, including when the held instance may have expired; a resume failure alone does not prove deletion. Automatic replacement for a policy mismatch was not chosen because another caller may still use the instance. Capture invalidation is a separate deletion path: replacement under any policy requires that deletion to succeed. The live regression verifies equivalent-policy reuse, mismatch refusal, continued availability under the original policy, and a correctly denying CLOSED replacement after explicit disposal.
