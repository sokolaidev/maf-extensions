...
- **`test_conformance_coverage.py`** — a package whose backend declares `Capability.FILES_OUT` has to call `maf_sandbox.conformance`'s suite from its own tests. Two backends written against the prose alone shipped the same confinement escape, twice each ([#142](https://github.com/sokolaidev/maf-extensions/issues/142)); the probes are what that cost bought, and this is what keeps a third backend from being held to prose again.
