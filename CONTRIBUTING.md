...
The hooks install into the git common directory, so one `uv run pre-commit install` in the main checkout covers every worktree of it; a worktree you check out on its own needs the install once in itself.
